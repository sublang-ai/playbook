...
# Linked Runtime Contract

This normative companion of [FSM-to-Runtime Linking](link.md) owns the runtime behavior delegated to the shared factory and required of bespoke implementations.
Its runtime sections are unchanged by the definition split.
The compiler's [inputs](link.md#linker-inputs), [role identity](link.md#role-identity), [output contract](link.md#output), and [compiled execution](link.md#compiled-execution) remain in the entry definition.
Section references to those concerns refer to that entry definition; every other section reference refers to the runtime sections below.
