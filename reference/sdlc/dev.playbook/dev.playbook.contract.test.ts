...
// `dev.playbook.d.ts`, not on a structural assignability check (which
// a same-shaped local redefinition would also pass).
const dts = readFileSync(
  fileURLToPath(new URL('./dev.playbook.d.ts', import.meta.url)),
  'utf8',
);
...
describe('DEV re-exports the shared runtime contract types (PBRT-36)', () => {
