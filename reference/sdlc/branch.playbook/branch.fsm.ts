...
const CREATE_BRANCH_RESULTS = {
  branched:
    'Coder created the new branch from the current commit and checked it out, reporting its exact name and a concise summary of the issue and its comments, or of the request when no issue is named; the absence of a reported obstacle is not support. Output shall include `branch: <exact branch name>`, `baseRevision: <repository revision>`, and `issueSummary: <concise summary>`.',
  refused:
    'Coder created nothing and reported the failure with its reason: the working tree is not clean, `gh` is not authenticated, the named issue does not exist or cannot be read, or a branch with that name already exists locally or on the remote. Output shall include `coderOutput: <verbatim final text>`.',
  needsBossReply:
...
const STATE_DESCRIPTIONS = {
  ...
  failed:
    'The branch workflow failed and is waiting for a new development request.',
  branched:
...
function isBranched({ event }: { event: unknown }): boolean {
  return (
    outputGuard(event, 'branched') &&
...
function isRefused({ event }: { event: unknown }): boolean {
  return (
    outputGuard(event, 'refused') &&
...
const machineSetup = setup({
  ...
  guards: {
    ...
    resumesCreateBranch: ({ context, event }) =>
  ...
  actions: {
    ...
    rememberPendingQuestion: assign(({ event }) => {
      ...
      return {
        pendingBossQuestion: {
          questionId: 'createBranch' as const,
          resumeStateId: 'createBranch' as const,
          sourceItem: 'BRANCH-1' as const,
    ...
    rememberMalformedPlayerOutput: assign({
      lastError: () =>
        new Error('Coder result for BRANCH-1 did not match an available outcome.'),
    }),
  },
});
  ...
  output: ({ context }): BranchPlaybookOutput => {
    if (context.completion === 'branched') {
      if (
        ...
        throw new Error(
          'BRANCH reached branched without the exact branch name, a receipt-observed base revision, and an issue summary',
        );
      }
      return {
        status: 'branched',
