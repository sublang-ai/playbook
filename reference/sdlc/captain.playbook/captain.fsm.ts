...
export type CompletedCallResult =
    | { readonly playbookId: string; readonly status: 'ok'; readonly output?: JsonValue }
    | {
          readonly playbookId: string;
...
type CaptainContext = {
    ...
    lastError?: CompactError & { readonly stack?: string };
};
...
const captainActor = fromPromise<CaptainOutput, CaptainInput>(async () => {
    throw new Error('captain actor must be provided by the runner');
});

const playbookActor = fromPromise<PlaybookOutput, PlaybookInput>(async () => {
...
function isFinitePlan(value: unknown): value is readonly JsonValue[] {
...
function validResponse(event: unknown, guard: 'direct' | 'final'): boolean {
    const output = outputOf(event);
    return isPlainObject(output) && output.guard === guard && isNonEmptyString(output.response);
}

function validQuestion(event: unknown, guard: 'question' | 'followUpQuestion' | 'needsBossReply'): boolean {
    const output = outputOf(event);
    return isPlainObject(output) && output.guard === guard && isNonEmptyString(output.question);
}

function validPlannedCall(event: unknown, guard: 'delegation' | 'continuing'): boolean {
    const output = outputOf(event);
    return (
...
function validDynamicCall(context: CaptainContext, event: unknown, guard: 'delegation' | 'continuing'): boolean {
    if (!validPlannedCall(event, guard)) return false;
    const output = outputOf(event);
    if (!isPlainObject(output)) return false;
    const playbookId = output.nextPlaybookId as string;
    const text = output.nextPlaybookInput as string;
    return (
...
function validContinuingCall(context: CaptainContext, event: unknown): boolean {
    if (!validDynamicCall(context, event, 'continuing')) return false;
    const output = outputOf(event);
    return (
        isPlainObject(output) &&
...
function normalizeError(value: unknown): CompactError & { readonly stack?: string } {
...
function authoredChildResult(event: unknown): { status: 'aborted' | 'error'; error: CompactError } | undefined {
    const outer = errorOf(event);
    if (!(outer instanceof Error)) return undefined;
    const result = (outer as Error & { readonly result?: unknown }).result;
    if (!isPlainObject(result) || (result.status !== 'aborted' && result.status !== 'error')) return undefined;
...
function captainInput(
    ...
    return {
        ...
        prompt,
        result,
        bossIntent: context.bossIntent as string,
...
function bossInterrupts(ids: readonly ['routing']) {
    return ids.map((id) => ({
        ...
        target: `#${id}` as const,
        reenter: true,
        actions: 'setInterruptedIntent' as const,
    }));
}
...
export const captainMachine = setup({
    ...
    actions: {
        setBossIntent: assign(({ event }) => {
            if (event.type !== 'BOSS_INTENT') return {};
            return {
        ...
        setInterruptedIntent: assign(({ event }) =>
            event.type === 'BOSS_INTERRUPT'
                ? {
                      bossIntent: event.bossIntent,
                      remainingPlan: [],
        ...
        rememberPlannedCall: assign(({ context, event }) => {
            const output = outputOf(event);
            if (!isPlainObject(output) || !isFinitePlan(output.remainingPlan) || !isNonEmptyString(output.nextPlaybookId) || !isNonEmptyString(output.nextPlaybookInput)) return {};
            return {
        ...
        setBossReply: assign(({ event }) =>
            event.type === 'BOSS_REPLY' && isNonEmptyString(event.answer)
        ...
        appendSuccessfulCall: assign(({ context, event }) => {
            const output = outputOf(event);
            if (!isJsonValue(output) && output !== undefined) return {};
            const completed: CompletedCallResult = output === undefined
                ? { playbookId: context.nextPlaybookId as string, status: 'ok' }
                : { playbookId: context.nextPlaybookId as string, status: 'ok', output: output as JsonValue };
        ...
        appendAuthoredChildFailure: assign(({ context, event }) => {
            ...
            if (!child || !context.nextPlaybookId) return {};
            return {
                completedCallResults: [
    ...
    output: ({ context }) => ({ response: context.response as string }),
    on: {
        ...
        callingPlaybook: {
            ...
            invoke: {
                ...
                input: ({ context }): PlaybookInput => ({
                    ...
                    playbookId: context.nextPlaybookId as string,
                    text: context.nextPlaybookInput as string,
