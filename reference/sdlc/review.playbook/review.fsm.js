...
export const reviewMachine = setup({
    ...
    guards: {
        ...
        rejectedAll: ({ event }) => {
            const output = outputOf(event);
            return (output.guard === 'rejectedAll' && isNonEmptyString(output.coderOutput));
        },
        ...
        resumeReviewInitial: ({ context, event }) => event.type === 'BOSS_REPLY' &&
        ...
        resumeAddressFindings: ({ context, event }) => event.type === 'BOSS_REPLY' &&
            event.answer.trim().length > 0 &&
            context.pendingBossQuestion?.resumeStateId === 'addressFindings' &&
            (event.questionId === undefined ||
                event.questionId === context.pendingBossQuestion.questionId),
        resumeReviewAfterCommit: ({ context, event }) => event.type === 'BOSS_REPLY' &&
            event.answer.trim().length > 0 &&
            context.pendingBossQuestion?.resumeStateId === 'reviewAfterCommit' &&
            (event.questionId === undefined ||
                event.questionId === context.pendingBossQuestion.questionId),
        resumeReviewAfterRebuttal: ({ context, event }) => event.type === 'BOSS_REPLY' &&
            event.answer.trim().length > 0 &&
