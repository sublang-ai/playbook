...
export type PlayerOutput =
  ...
  | { readonly guard: 'notPublished'; readonly coderOutput: string }
...
export type ScriptOutput =
  | { readonly guard: 'checksPassed'; readonly exitStatus: number }
  | { readonly guard: 'checksFailed'; readonly exitStatus: number }
  | { readonly guard: 'fixPublished'; readonly exitStatus: number }
  | { readonly guard: 'fixNotPublished'; readonly exitStatus: number }
  | { readonly guard: 'checksStillFailing'; readonly exitStatus: number }
  | { readonly guard: 'merged'; readonly exitStatus: number }
  | { readonly guard: 'mergeRefused'; readonly exitStatus: number }
  | { readonly guard: 'localDefaultUpdated'; readonly exitStatus: number }
  | { readonly guard: 'localDefaultNotUpdated'; readonly exitStatus: number };
...
export type PrEvent =
  | { readonly type: 'START_PR'; readonly callerInput: string }
  | {
...
const OPEN_PULL_REQUEST_RESULTS = {
  opened:
    'Coder pushed the branch with its upstream set and reported the open pull request for it. Output shall include `pullRequest: <pull request number>` and `pullRequestUrl: <pull request URL>`.',
  notPublished:
    'Coder reported that the branch could not be published or its pull request could not be opened, with the reason. Output shall include `coderOutput: <verbatim final text>`.',
...
const WAIT_FOR_CHECKS_RESULTS = {
  checksPassed:
    "The command exited with status zero: the pull request's checks passed, or the repository still reported no checks after a brief wait for them to register.",
  checksFailed:
    "The command exited with a nonzero status: the pull request's checks failed.",
} as const;
...
const WAIT_FOR_CHECKS_AFTER_FIX_RESULTS = {
  checksPassed:
    "The command exited with status zero: the pull request's checks passed after the fix, or the repository still reported no checks after a brief wait for them to register.",
  checksStillFailing:
    "The command exited with a nonzero status: the pull request's checks are still failing after the one fix attempt.",
} as const;
...
const MERGE_PULL_REQUEST_RESULTS = {
  ...
  mergeRefused:
    'The command exited with a nonzero status: GitHub refused the merge, or the merge landed but the local switch to the default branch or the branch deletion failed.',
} as const;

const UPDATE_LOCAL_DEFAULT_RESULTS = {
  localDefaultUpdated:
    'The command exited with status zero: the local default branch is fast-forwarded to the merged head.',
  localDefaultNotUpdated:
    'The command exited with a nonzero status: the local default branch was not fast-forwarded to the merged head.',
} as const;
...
const STATE_DESCRIPTIONS = {
  ...
  openPullRequest:
    'Coder is publishing the branch and opening its pull request.',
  waitForChecks:
    "Waiting for the pull request's checks to settle before any fix attempt.",
  fixChecks:
    'The CODE playbook is fixing the cause of the red checks on the checked-out branch.',
  publishFix:
    "Publishing the fix to the pull request and waiting for the pull request's head to reach the pushed commit.",
  waitForChecksAfterFix:
    "Waiting for the pull request's checks to settle after the one fix attempt.",
  mergePullRequest:
    'Merging the pull request into the repository default branch.',
  updateLocalDefault:
    'Fast-forwarding the local default branch to the merged head.',
...
function actorOutput(event: unknown): Record<string, unknown> | undefined {
  if (!isRecord(event)) return undefined;
  return isRecord(event.output) ? event.output : undefined;
}

function outputGuard(event: unknown, guard: string): boolean {
...
function isOpened({ event }: { event: unknown }): boolean {
  return (
    outputGuard(event, 'opened') &&
    outputString(event, 'pullRequest') !== undefined &&
    outputString(event, 'pullRequestUrl') !== undefined
  );
}

function isNotPublished({ event }: { event: unknown }): boolean {
  return (
    outputGuard(event, 'notPublished') &&
...
function scriptExitStatus(event: unknown): number | undefined {
  const value = actorOutput(event)?.exitStatus;
  return Number.isInteger(value) ? (value as number) : undefined;
}

function zeroExitGuard(
  guard: ScriptOutput['guard'],
): (args: { event: unknown }) => boolean {
  return ({ event }) =>
    outputGuard(event, guard) && scriptExitStatus(event) === 0;
}

function nonzeroExitGuard(
  guard: ScriptOutput['guard'],
): (args: { event: unknown }) => boolean {
  return ({ event }) => {
    const exitStatus = scriptExitStatus(event);
    return (
      outputGuard(event, guard) && exitStatus !== undefined && exitStatus !== 0
    );
  };
}
...
type AuthoredChildFailure =
  ...
  // bridge rejects it through the same error path, so PR recognizes the
...
export function fixChecksCallText(context: PrContext): string {
  return quotedRelay([
...
function pullRequestIdentity(context: PrContext): {
  readonly pullRequest: string;
  readonly pullRequestUrl: string;
} {
  if (
    !isNonEmptyString(context.pullRequest) ||
...
const machineSetup = setup({
  ...
  guards: {
    ...
    resumesOpenPullRequest: ({ context, event }) =>
  ...
  actions: {
    ...
    completeNotPublished: assign(({ event }) => ({
      completion: 'not-published' as const,
    ...
    rememberPendingQuestion: assign(({ event }) => {
      ...
      return {
        pendingBossQuestion: {
          questionId: 'openPullRequest' as const,
          resumeStateId: 'openPullRequest' as const,
          sourceItem: 'PR-1' as const,
          asker: { kind: 'role' as const, roleId: 'coder' as const },
          question,
        },
        bossReply: undefined,
      };
    ...
    completeWithInsufficientCodeResult: assign(({ event }) => ({
      completion: 'fix-failed' as const,
    ...
    completeFixNotPublished: assign({
      completion: 'fix-not-published' as const,
    }),
    ...
    rememberMalformedPlayerOutput: assign({
      lastError: () =>
        new Error('Coder result for PR-1 did not match an available outcome.'),
    }),
    rememberMalformedScriptOutput: assign(
      (_args, params: { readonly sourceItem: PrScriptSourceItem }) => ({
        lastError: new Error(
          `Script result for ${params.sourceItem} did not match a declared outcome.`,
        ),
      }),
    ),
  },
...
export const prMachine = machineSetup.createMachine({
  ...
  initial: 'ready',
  context: () => ({}),
  output: ({ context }): PrPlaybookOutput => {
