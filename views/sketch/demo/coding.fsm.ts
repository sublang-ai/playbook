...
// Vendored from `demo/sdlc/src/.playbook/coding.fsm.ts` for the standalone
// views/sketch demo. The upstream copy is the source of truth; this copy
// inherits the upstream's existing TS strictness gaps, so we skip type
// checking here and let the demo wiring (`coding-demo.ts`) carry the strict
// typing burden via the public `codingMachine`/`Captain*`/`CodingEvent`
// surface.
// @ts-nocheck
