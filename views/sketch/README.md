...
![mid-run snapshot of the coding.fsm.ts visualizer with reviewCodeCommit active and the planAndImplement→reviewCodeCommit branch-0 edge fired](docs/coding-fsm-mid-run.svg)

The snapshot above is auto-generated by `npm run capture:screenshot`,
which calls `extractGraph` + `renderSketchToString` (DR-002 §3, §5)
against the vendored `coding.fsm.ts`, inlines the theme styles, and
applies `.active` / `.fired` classes to a chosen state and edge — the
same DOM the live demo produces mid-run. Re-run after machine or theme
changes.
