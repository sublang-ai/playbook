...
- [ ] cligent Captain player calls accept an explicit resume token or fresh-session selection.
- [ ] The shared runtime contract and linker specification expose session and resume types.
- [ ] CODE and DISCUSS trace every runtime-boundary operation and maintain per-player resume tokens.
- [ ] The Playbook Captain generates session UUIDs and forwards resume selections and tokens.
- [ ] Integration tests cover fresh-session isolation, token rotation/clearing, trace order, and lifecycle identity.
