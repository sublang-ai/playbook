...
- `--player reviewer=codex:gpt-5.5:xhigh` reaches the injected agent factory with that model and effort; `--captain claude::high` sets effort with the default model; an unsupported effort exits `1` naming the adapter's supported values before any agent runs.
