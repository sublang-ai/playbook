...
- [ ] `views/sketch/src/sketch.ts` — exports values `mountSketch`, `extractGraph`, `renderSketch`, `renderSketchToString`, `applySketchTelemetry`, `fromXStateActor`, `fromEventSource`; exports types `SketchTelemetry`, `SketchSource`, `SketchActorSource`, `SketchGraph`.
- [ ] `views/sketch/src/graph.ts` — machine → graph (with `branchIndex`, `targetIndex`, `guardKey`); pure, DOM-free.
...
- `SketchSource` shape: a unit test asserts `fromXStateActor` returns an object satisfying `SketchActorSource` (has `subscribe`, `dispose`, `emitLatestActive`); `fromEventSource` returns a `SketchSource` (has `subscribe`, `dispose`; no `emitLatestActive`). `subscribe` returns an unsubscribe function whose call detaches the listener. `dispose()` is idempotent; subscribing after `dispose` is a no-op.
- Latest-active retention (matcher): a unit test creates `fromXStateActor` and subscribes a no-op listener (so the source is live but no live consumer is recording), drives the actor through several state changes, then subscribes a recording listener and calls `source.emitLatestActive()`. The recording listener receives exactly one `active` event whose `activeStateIds` matches the actor's current snapshot, with a fresh (later) `seq`. Calling `emitLatestActive()` before any `active` has been produced is a no-op (no callbacks). No stale `fired` events are ever replayed.
