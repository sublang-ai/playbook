...
A frame shall participate only when its runtime exposes the `retainedGenerationMetadata` capability marker from [[playbook-runtime-34](playbook-runtime.md#playbook-runtime-34)]; a capability-less root shall emit `clear`; and, for an outcome that would otherwise `retain`, when the root stack selected by that outcome contains a capability-less descendant, the shell shall retain the last complete turn-start candidate or, when the turn began with such a descendant already live or the root began during the turn, emit no update for that root, and shall never retain a partial stack.
