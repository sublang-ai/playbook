...
Outside that no-prior-generation case, a runtime claiming the marker but unable to supply a safe work-bearing pre-terminal generation or a stable terminal state id shall fail the settlement boundary rather than preserve stale retention or classify the terminal as clean.
