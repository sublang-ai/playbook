...
A frame shall participate only when its runtime exposes the `retainedGenerationMetadata` capability marker from [[playbook-runtime-34](playbook-runtime.md#playbook-runtime-34)]; if any live frame lacks the marker, the shell shall emit `clear` for that root and retain no partial stack.
...
When the root remains parked, the shell shall emit `retain` with its current complete generation; when dismissal removes the root, it shall emit `retain` with the captured generation; and when a terminal result removes the root, it shall emit `retain` with the captured generation exactly when the terminal state's stable id belongs to the root runtime's immutable `unfinishedFinalStateIds` ([[playbook-runtime-34](playbook-runtime.md#playbook-runtime-34)]), otherwise `clear`.
An initialized root that has not yet reached a post-input quiescent settlement shall not count as carrying unfinished work; if it reaches a declared unfinished terminal first, the shell shall fail the settlement boundary rather than retain its initial state or emit `clear`.
A runtime claiming the marker but unable to supply a safe work-bearing pre-terminal generation or a stable terminal state id shall fail the settlement boundary rather than preserve stale retention or classify the terminal as clean.
