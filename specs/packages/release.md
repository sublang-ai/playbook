...
The living-pointer resolution case shall retain its assertions and use an explicit 30-second per-test timeout budget, rather than widen the global timeout, because packing and scanning the full Markdown set can exceed Vitest's five-second default on supported Node 20 runners.
...
The schema-gated construction-type case shall retain its assertions and use an explicit 30-second per-test timeout budget, rather than widen the global timeout, because building the synthetic TypeScript consumer program can exceed Vitest's five-second default on supported Node 20 runners.
