...
- the installed Captain, CODE, REVIEW, DECIDE, DEV, BRANCH, and PR playbook subpaths construct, Captain, CODE, REVIEW, DEV, BRANCH, and PR expose retained-snapshot adoption while bespoke DECIDE omits it ([[release-28](#release-28)]), every packed file other than the manifest is byte-identical to the repository's own, the deterministic source-preservation check passes, and each compiled artifact's conformance suites pass with their declared coverage named among those that ran; and
