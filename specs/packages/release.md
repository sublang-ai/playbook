...
The published package shall expose the generic `playbook` executable through `package.json` `bin` and the CODE, REVIEW, DECIDE, and DEV playbook and registry modules through public `exports['./<id>/playbook']` and `exports['./<id>/registry']` subpaths, all backed by files listed in `files`, as public semver-stable surfaces.
...
The package shall also ship the authored CODE, REVIEW, DECIDE, and DEV sources `reference/sdlc/code.md`, `reference/sdlc/review.md`, `reference/sdlc/decide.md`, and `reference/sdlc/dev.md` beside their compiled artifacts, so a host can display or recompile the bundled playbooks from source.
...
That module shall expose only the narrow facade of [[playbook-cli-87](playbook-cli.md#playbook-cli-87)] over the CLI host's single repository-effect implementation, while that implementation's lease, session-record, catalog, recovery, and Captain-lifecycle members shall remain unexported.
...
9. **Nested cligent floor.** The nested installed `@sublang/cligent` shall satisfy the caret range the packed manifest declares and expose the complete enumerated release-floor capability set through its root and `@sublang/cligent/tmux-play` public specifiers resolved from that nested copy ([[release-14](#release-14)]).
   The guard shall prove the runtime presence and exact call signatures of `launchManagedTmuxPlay` and `runManagedTmuxPlaySession`, plus the exact usable type and optionality of `CaptainContext.emitReply`, `CaptainRunResult.resumeToken`, `Captain.prepareDispose`, the player and Captain call-option parameters, each continuation and complete-setting option, every complete setting and tuning selection including optional boolean `AgentCallSettings.fastMode`, the root `assertFastModeSupported` type and callable runtime export whose supported-versus-unsupported results agree with Cligent's own public `FAST_MODE_SUPPORT` descriptor, the typed settings rejection and predicate, the required work-directory cleanup-authority member on both managed launch context and direct session options, both managed attach activation members, and the prepared attach parameter.
...
The DEV case shall fail unless every Analyst planning receipt is `unchanged`, the accepted planning outcome is `code` with no Boss-question suspension and no DECIDE call, nested CODE implements and commits the fixture requirement, nested REVIEW approves it, and the worktree is clean.
...
the playbook and registry subpaths for CODE, REVIEW, DECIDE, and DEV,
...
four workflow registry `.js` and `.d.ts` artifacts
...
- the installed executable answers top-level and `run` help with the fresh, continuation, and uncertain-recovery grammar, and `--list` names the CODE, REVIEW, DECIDE, and DEV registries;
...
- the installed Captain, CODE, REVIEW, DECIDE, and DEV playbook subpaths construct, Captain, CODE, REVIEW, and DEV expose retained-snapshot adoption while bespoke DECIDE omits it ([[release-28](#release-28)]), every packed file other than the manifest is byte-identical to the repository's own, the deterministic source-preservation check passes, and each compiled artifact's conformance suites pass with their declared coverage named among those that ran; and
