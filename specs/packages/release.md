...
Its declaration shall own the opaque replay-payload types used by that
surface ([[playbook-cli-74](playbook-cli.md#playbook-cli-74)]) and shall resolve under ordinary consumer declaration checking
...
- an external package declaring only `@sublang/playbook` type-checks with `skipLibCheck: false` against the packed self-contained session-store declaration and, through that facade alone, opens a store, lists and reads a CLI-written session record, acquires and releases its lease, and appends and reads back stream entries that satisfy the frozen envelope, sequence, and readable-prefix contract, carry no resume token, and reject a below-schema record rather than migrate it (verifying [[release-33](#release-33)] and [[release-28](#release-28)]).
