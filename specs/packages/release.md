...
The declared floor shall expose both the stock tmux-play child-launch surface used by the raw and diagnostic forms of [[playbook-cli-1](playbook-cli.md#playbook-cli-1)] and the managed prepared-launch, gated-input, terminal-record, buffered-reply, activation-abort, synchronous native-client hand-off, cancellation, and failure-atomic shutdown surfaces used by [[playbook-cli-49](playbook-cli.md#playbook-cli-49)].
...
Before a DR-032 candidate releases, the pin shall be at least 0.21.0 and shall expose the segmented-id grammar, empty-roster host shape, atomic complete per-call settings, and typed settings rejection above.
