...
A schema-version-3 record may omit `retainedGenerations`; the store shall interpret absence as an empty map for explicit selection, latest-session selection, ordinary continuation, retention updates, and adoption-predecessor use while preserving the absent member through uncertain marking, retry, and exact discard.
