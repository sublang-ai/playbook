...
Where a host appends through a replay writer whose numeric live status has `incomplete: false` under [[playbook-cli-83](#playbook-cli-83)], appends shall be awaited in record order with the next sequence one past `lastReadableSeq`.
When the writer first publishes the stream pathname, it shall synchronize the containing sessions directory before accepting the first append.
...
A normal release of a numerically initialized writer shall return its final numeric status after the applicable checkpoint attempt, including an incomplete status when that attempt fails.
...
On the first transition to an incomplete replay status under [[playbook-cli-76](#playbook-cli-76)] or [[playbook-cli-83](#playbook-cli-83)], each CLI front end shall attempt exactly one direct stderr write for that acquired writer lease, outside the record dispatcher and presentation gate: substituting `JSON.stringify(sessionId)` for `<session>`, the headless host shall write `playbook run: warning: replay history for session <session> may be incomplete; recording has stopped\n`, and the managed child shall write `playbook: warning: replay history for session <session> may be incomplete; recording has stopped\n`.
The front end shall mark that warning reported before the write attempt, swallow a warning-write failure, never retry or tee the diagnostic, and warn once again only when a successor lease independently becomes incomplete; the facade shall emit no diagnostic because its caller owns presentation through the returned status.
Successful teeing shall change neither buffered stdout nor stderr; with a writable warning sink, degraded teeing shall add exactly that one line and no other output, while a warning-write failure shall be swallowed without retry; neither path shall change the turn, reply, settlement, attachment, release, or exit outcome of [[playbook-cli-18](#playbook-cli-18)] and [[playbook-cli-20](#playbook-cli-20)].
...
For monotonic incremental reads, the store shall cache only a fully validated newline-terminated boundary — the opened-file identity, observed owner-token generation of the canonical lease under [[playbook-cli-23](#playbook-cli-23)], byte offset immediately after the final validated line feed, and final sequence — and, while that identity and generation are unchanged and the completed prefix is preserved, validate only bytes from that boundary and require the first new completed envelope to carry the next sequence.
...
Replacement, truncation into the validated prefix, or an observed canonical-lease owner-token transition, including a transition to or from no owner, shall invalidate the cursor and require validation from byte zero; if that validation does not preserve a complete prefix this store already observed, the live store shall reject the rollback, while a new store with no prior observation may accept the actual retained prefix.

#### playbook-cli-83

After acquiring the valid exclusive session lease of [[playbook-cli-23](#playbook-cli-23)], the replay writer shall rescan the stream from byte zero without reusing a follower cursor, validate its complete retained prefix, and seed both `lastReadableSeq` and `lastDurableSeq` from the final complete sequence or `0`.
It shall then repair and synchronize any torn final line while holding that lease; absence of a torn tail, or successful repair and synchronization, shall complete initialization with `incomplete: false`, while repair or repair-synchronization failure shall enter the numeric latch-and-stop state of [[playbook-cli-76](#playbook-cli-76)].
...
| Independent follower | Absent and empty streams each return no entries with exactly `lastReadableSeq: 0`; an independent reader then first validates a nonempty complete prefix, and after another store's lease appends a complete but uncheckpointed line, a monotonic incremental read reads and validates only bytes from its cached complete-line boundary, returns exactly the requested suffix with `lastReadableSeq` equal to the whole prefix's final sequence, and exposes no durable-sequence or incomplete-state member; acquiring the lease after that follower read still rescans from byte zero rather than seeding from follower state ([[playbook-cli-82](#playbook-cli-82)], [[playbook-cli-83](#playbook-cli-83)]). |
