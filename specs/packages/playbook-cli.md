...
A failure or shutdown after guarded fresh initialization but before the first marker or managed readiness may delete only that launch's byte-exact empty turn-zero record after safe host cleanup while the launch still owns it; a changed, nonempty, or ambiguously owned target shall remain durable.
...
The initial scan shall only nominate the predecessor; the store shall acquire that candidate's exclusive lease, authoritatively reread it, and rescan while that lease is held before publishing or returning a detached selection.
For detached selection the candidate shall remain the same newest eligible boundary, while guarded initialization shall decline a candidate that disappeared or was superseded by another newest valid boundary, publish the proposed empty target strictly after every boundary it observed, and neither retry nor fall through.
During guarded initialization an initially nominated candidate proved to have a live lease shall likewise publish the proposed empty target and yield no transfer without falling through or blocking launch, while detached selection shall yield no predecessor; foreign-host, permission-unknown, malformed, or otherwise unprovable ownership of that nominated candidate shall fail closed with the target absent or unchanged as applicable.
...
A successful exchange shall move the complete detached retention map into the target's first record and clear the complete source map, without transferring a compatible subset or changing either record's snapshot, projections, lifecycle state, generation contents, or unrelated members except `updatedAt`; both publications shall use canonical schema version `3`, source clear shall advance its prior timestamp, and target install shall advance strictly after both its proposed timestamp and the source clear so the new owner is the next deterministic predecessor under [[playbook-cli-23](#playbook-cli-23)].
...
After safe host cleanup, either front end shall retract its own byte-exact empty turn-zero target when a later pre-marker or pre-readiness failure aborts launch, but shall preserve a changed, nonempty, or ambiguously owned target.
...
The suite shall further fail unless a nested multi-root map moves whole and detached from one fresh settled record to another while preserving every generation identity and unrelated record member, advances source then target monotonically under a tied or regressed clock, leaves the target as the selected predecessor, and restores that target boundary exactly after a later uncertain marker is discarded; a detached exchange rejects every exercised root-envelope mismatch and missing descendant catalog before either write while guarded initialization declines root-option drift with the source byte-exact and an empty target, every permitted Captain, player, tuning, descendant-option, role-binding, command, and intent difference remains accepted, detached stale or newly superseded selection and target reuse or owner loss reject under authoritative reread, guarded predecessor disappearance or supersession declines with an empty newest target, and two contenders transfer at most one copy (verifying [[playbook-cli-51](#playbook-cli-51)] and [[playbook-cli-53](#playbook-cli-53)]).
