...
caller, consuming launcher-owned session and overlay arguments and forwarding every other user-supplied argument verbatim.
...
The help text shall state that a fresh managed `playbook` launch creates and reports a durable logical Captain session id, and shall identify `playbook --session <id>` and `playbook run --session <id> [reply]` as the two presentation-specific ways to reopen one logical Captain session through [[playbook-cli-49](#playbook-cli-49)].
...
When a managed interactive `playbook` launch reaches host initialization without a session selector, the front end shall generate a fresh public logical session UUID, acquire its exclusive writer lease, initialize the same shell as `playbook run` under [[playbook-cli-20](#playbook-cli-20)], atomically persist its complete turn-zero settled record under [[playbook-cli-23](#playbook-cli-23)], and print exactly `playbook: session <id>` to stderr before attaching or accepting Boss input.
Where a settled logical Captain session exists, when the user invokes `playbook --session <id>`, the interactive front end shall acquire that session's exclusive writer lease under [[playbook-cli-23](#playbook-cli-23)], restore the same shell and stored working directory as `playbook run --session <id>`, resolve the current shared config plus any `--with` overlays under [[playbook-cli-22](#playbook-cli-22)], and expose the retained public id through the same operational stderr line before attaching or accepting Boss input.
Either managed interactive form shall retain ownership across turns until a durable hand-off or process exit, and every non-empty Boss turn shall cross the write-ahead and atomic-settlement boundary of [[playbook-cli-23](#playbook-cli-23)] before its reply is presented.
The managed interactive command shall reject `--continue` and uncertain-session recovery flags; a session-selected form shall reject a combined raw `--config` and repeated or otherwise combined session selectors, and shall never create a second logical session merely because presentation changed.
An initialization, initial-snapshot, or initial-record failure shall print a diagnostic and exit nonzero before reporting the id, attaching, or accepting Boss input.
...
Released Captain-session record schema `2`, shell snapshot schema `1`, and runtime snapshot schemas `1` and `2` shall receive an incompatible-player-identity diagnostic rather than an inferred conversion to schema version `3`.
...
Before a fresh managed interactive session created through [[playbook-cli-49](#playbook-cli-49)] accepts Boss input, it shall atomically persist its complete settled turn-zero record under the same store contract.
A managed interactive session, whether created fresh or reopened through [[playbook-cli-49](#playbook-cli-49)], shall atomically replace its settled record with the same uncertain marker before agent work for each submitted non-empty Boss turn and replace that marker with the complete settled session before presenting the reply, while retaining its exclusive lease across turns until durable hand-off or exit.
...
The record shall be a closed, strictly plain-JSON Captain-session schema version `3`, distinct from the released direct-run-only and root-owned-player boundaries.
A settled record shall contain exactly `schemaVersion`, `kind`, `state`, `sessionId`, `createdAt`, `updatedAt`, `cwd`, `structuralProjection`, `lastAppliedExecutionProjection`, and `snapshot`; an uncertain record shall contain those same common fields plus exactly one `uncertain` member.
...
When the integration suite creates a fresh managed interactive session and a fresh headless session and reopens each through both front ends, it shall fail unless the interactive launch durably persists a complete turn-zero settled record before printing exactly one matching `playbook: session <id>` stderr line and accepting input; every non-empty interactive turn exposes its exact uncertain record before agent work and its complete settled replacement before presenting the reply; `playbook --session <id>` and `playbook run --session <id>` preserve the same public id, shell state, and player ledger; each applies current model and effort with the prior token; interactive ownership spans its attached lifetime; headless ownership spans one settled turn; concurrent ownership rejects before agent work; and neither age, completed roots, turn count, nor a compatible model change makes the session ineligible (verifying [[playbook-cli-22](#playbook-cli-22)], [[playbook-cli-23](#playbook-cli-23)], and [[playbook-cli-49](#playbook-cli-49)]).
The suite shall reject interactive `--continue` and uncertain recovery flags, raw `--config` combined with a session selector, and combined or repeated selectors before config resolution, lease acquisition, or host work (verifying [[playbook-cli-49](#playbook-cli-49)]).
