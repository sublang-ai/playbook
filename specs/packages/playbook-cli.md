...
Where a host appends to a replay stream, it shall hold that session's exclusive lease of [[playbook-cli-23](#playbook-cli-23)], and appends shall be awaited in record order with the next sequence one past the last complete sequence the stream retained ([[playbook-cli-74](#playbook-cli-74)]).
...
The writer shall synchronize accumulated completed lines as one content checkpoint after each successful private session-record settlement of [[playbook-cli-23](#playbook-cli-23)] and before a normal lease release returns; only a successful checkpoint shall advance the reported last durable sequence.
When an append, first-publication directory synchronization, or content checkpoint fails, the writer shall latch the stream incomplete at the last durable sequence and stop appending for the remainder of the session so its readable content stays a clean contiguous prefix, but shall neither fail nor undo the agent turn, a successful private session-record settlement, or an otherwise valid lease release.
Where an append or content-checkpoint failure leaves complete visible lines beyond the last durable sequence, the writer shall not roll them back; a successor lease shall accept whichever complete prefix the filesystem retained and continue one past its last sequence.
After latching, that store instance's stream reader shall return records only through its reported last durable sequence, report incompleteness at that same boundary, and leave later complete bytes untouched, while a newly opened store shall scan the actual complete retained prefix and report no inherited latch.
A lease-free reader shall leave the file byte-identical and repair nothing; absent that store instance's live latch it shall return the readable prefix of [[playbook-cli-74](#playbook-cli-74)], while after a latch the preceding durable-boundary rule shall govern, and repairing a torn final line shall be lawful only under a held lease.
...
| --- | --- |
| Public surface and listing | The JavaScript and declaration exports and every returned store and lease member equal the exact narrow-facade names and shapes; no lifecycle, validator, effect-ledger, staging, publication, or retirement member is reachable; the module graph proves the facade and both front ends depend on one private store and validator definition rather than parallel implementations; listing returns every valid canonical Playbook manifest while reporting every invalid canonical manifest without aborting; direct reading rejects a below-schema record; and stream-only or foreign-sidecar files remain unlisted and byte-identical ([[playbook-cli-73](#playbook-cli-73)]). |
...
| Fail-soft writer | Where either front end observes an entry that cannot become JSON-safe or a stream append, first-publication directory synchronization, settlement checkpoint, or release checkpoint fails, the turn, reply, successful private settlement, and otherwise valid release still complete; the readable file remains a clean contiguous prefix; no later record is appended; the live store reports incompleteness at the last durable sequence; and complete lines already visible beyond that boundary are not rolled back ([[playbook-cli-75](#playbook-cli-75)], [[playbook-cli-76](#playbook-cli-76)], [[playbook-cli-77](#playbook-cli-77)]). |
| Torn tail and successor | A lease-free read returns only the newline-terminated prefix and leaves a torn file byte-identical; held-lease reading repairs and synchronizes only the torn final line; append without exact ownership is refused; after a live latch, that store's read returns only through the reported durable sequence while leaving later complete bytes untouched; after a normal or failed writer, a successor accepts the complete prefix actually retained and appends at its next sequence; and a newly opened store reports no prior process's live-only incomplete latch and reveals any retained complete lines beyond the former durable boundary exactly once to an incremental reader ([[playbook-cli-76](#playbook-cli-76)]). |
