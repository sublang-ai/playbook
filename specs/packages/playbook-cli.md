...
Retry shall first reconstruct every incomplete governed boundary under [[playbook-cli-63](#playbook-cli-63)]; while the authoritative ledger differs from the stored pre-turn snapshot it shall leave the record uncertain and the host closed before source-state restoration or any player call, and only the later whole-turn reconciliation policy may authorize restoring that snapshot, reusing the exact stored input and attempted config, durably incrementing the attempt before effects, and following the normal settlement boundary without applying current config or overlays.
