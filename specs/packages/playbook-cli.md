...
An explicitly selected released Captain-session record schema `2` or `3` shall receive a diagnostic naming its incompatible player identity or absent retained-generation state rather than an inferred conversion to record schema version `4`.
...
When implicit `--continue` scans the durable store, it shall report and skip each released Captain-session record schema `2` or `3` by exact session id and path with an archive-or-remove remedy; malformed or unsafe records and unknown schemas shall remain fail-closed.
...
The record shall be a closed, strictly plain-JSON Captain-session schema version `4`, distinct from the released direct-run-only, root-owned-player, and retention-less boundaries.
A settled record shall contain exactly `schemaVersion`, `kind`, `state`, `sessionId`, `createdAt`, `updatedAt`, `cwd`, `structuralProjection`, `lastAppliedExecutionProjection`, `snapshot`, and `retainedGenerations`; an uncertain record shall contain those same common fields plus exactly one `uncertain` member.
...
`--continue` shall select by canonical `updatedAt` with a deterministic session-id tie-break, report and skip a canonically named released schema-2 or schema-3 record as nonresumable, and fail closed on a malformed or unsafe record or unknown schema rather than silently falling back past corruption.
...
When the persistence integration suite exercises retained generations through real record files, the real Captain shell, both front-end settlement callers, and successive leases, the suite shall fail unless a nested root-to-leaf stack and both call-bridge halves round-trip as one detached unit across settlement, host and runtime disposal, reread, uncertain retry, and exact discard; a later settlement replaces only its named root; shell-derived clean completion clears only its named root; malformed, final, nonquiescent, cyclic, duplicate, unknown, or dangling stacks and duplicate or malformed updates reject without settlement; and pre-rename or post-rename durability faults expose only a complete old or new snapshot-generation pair (verifying [[playbook-cli-51](#playbook-cli-51)]).
