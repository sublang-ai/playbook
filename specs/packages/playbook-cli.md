...
For a fresh target, the child shall initialize the shared shell of [[playbook-cli-20](#playbook-cli-20)] and durably persist its complete turn-zero settled record before reporting readiness; for a selected target, it shall restore that shell and the stored working directory while applying the compatible current tuning selected by [[playbook-cli-22](#playbook-cli-22)].
...
Its exact `uncertain` member shall contain `baseUpdatedAt`, `input`, `attemptId`, `attemptNumber`, `markedAt`, and `attemptedExecutionProjection`, carrying the prior settled `updatedAt` or null for a fresh never-settled session, exact input, UUID attempt id, positive attempt number, marker timestamp equal to the root `updatedAt`, and exact execution settings about to be used.
A fresh headless marker shall use the initialized turn-zero execution projection as both its last-applied baseline and attempted projection; a continued marker shall retain the prior last-applied projection while recording the newly resolved current projection as attempted.
...
Persistence shall use an unpredictable same-directory `0600` exclusive temporary regular file, sync its contents, publish only a fresh session's first record — a headless uncertain marker or an interactive turn-zero settled record — with an atomic no-replace operation, replace every later same-session state transition by atomic rename, and sync the containing directory as required for the platform contract; a fresh UUID shall never overwrite an existing record.
...
Target Captain and player envelopes, model and effort selections, player identities, role-to-player bindings, effective commands, and intents shall not participate in that generation-envelope comparison.
