...
A scalar `captain` or `players.<player-id>` value shall name an adapter shorthand such as `claude` or `codex`; a full block shall follow the host tmux-play agent-block schema and shall carry its own adapter and default model, effort, fast mode, instruction, and permissions as needed.
Each `playbooks.<id>.roles.<role>` value shall be either a scalar player id or a block containing exactly `player` plus optional `model`, `effort`, and `fastMode`; each model or effort override shall be a nonempty concrete string or boolean `false` selecting `provider-default`, each `fastMode` override shall be a boolean whose `false` is a literal request rather than a provider-default selection, omission of any override shall inherit the top-level player default, and adapter, instruction, permissions, workspace, and tool settings shall be forbidden in a role binding.
Every binding shall resolve each model and effort field from its own override or the player default into an explicit concrete-value or provider-default selection and shall carry the effective fast mode when its override or the player default supplies one, so each resumed call can reapply complete tuning rather than inherit another role's provider state.
...
Whenever a Captain, player, or role supplies `fastMode`, the launcher shall delegate capability validation of that literal `true` or `false` to the installed cligent contract before registry preparation, import, readiness, or host work and shall maintain no adapter support list of its own ([DR-027](../decisions/027-runtime-compatibility-from-cligent.md)).
...
Discovery shall skip validated history-only and legacy records with their reasons; explicit legacy selection shall require `playbook migrate-session <id>` before execution [[playbook-cli-92](#playbook-cli-92)].
...
The command shall use current normalized model, effort, and optional fast mode for the stored identities; settings rejection shall fail without fresh fallback, while missing or definitively rejected continuity follows the shared hint/reset contract [[session-storage-6](session-storage.md#session-storage-6)] [[session-storage-8](session-storage.md#session-storage-8)].
...
Before registry preparation or import, shell restoration, hint consumption or repository reconciliation, continuation shall validate the shared replay/context boundary and path compatibility [[session-storage-4](session-storage.md#session-storage-4)] [[session-storage-9](session-storage.md#session-storage-9)].
...
An explicitly selected released Captain-session record schema `2` shall receive an incompatible-player-identity diagnostic rather than an inferred conversion to the current record schema.
...
When implicit `--continue` scans the durable store, it shall report and skip each fully validated released record schema `2`, `3`, or `4` and each recognized pre-release schema-`5` record by exact session id and path with the same applicable incompatibility or effect-authority-cutover reason as explicit selection and an archive-or-remove remedy; schema `6` shall report that explicit migration is required, complete settled schema `7` shall remain selectable, and history-only records shall report their stored reason; other malformed or unsafe records and unknown schemas remain fail-closed.
...
Each normalized `captain.options.playbooks.<id>` entry shall carry the canonical prepared `from`, normalized effective `command`, exact `roles: Readonly<Record<roleId, { playerId: string; model: TuningSelection; effort: TuningSelection; fastMode?: boolean }>>`, and an `options` slice built from every non-launcher key of the block; `from`, `command`, and `roles` shall not appear in the option slice.
...
The launcher shall also set `captain.options.sessionAgents` to exactly `{ captain: SessionAgent; players: Readonly<Record<playerId, SessionAgent>> }`, where `SessionAgent` is the normalized top-level default `{ adapter: string; model: TuningSelection; effort: TuningSelection; fastMode?: boolean; instruction?: string; permissions?: PermissionPolicy }`, `players` contains exactly referenced ids, and each role's independently resolved model, effort, and fast mode remain in its own binding; the shell uses the agent envelope plus that binding to form cligent's atomic complete-call settings for player calls under [[playbook-captain-10](playbook-captain.md#playbook-captain-10)], Captain calls under [[playbook-captain-31](playbook-captain.md#playbook-captain-31)], and durable envelopes under [[playbook-captain-41](playbook-captain.md#playbook-captain-41)].
...
- where no listed adapter is ready, the first row shall be seeded and the
  ...
  which lineup was written;
- the runtime-availability probe of
  [[playbook-cli-39](playbook-cli.md#playbook-cli-39)] shall take no part in
...
prompts.
Where the chosen adapter is `codex`, every seeded agent shall additionally
carry `permissions.writablePaths: ['.git']` so it can write git metadata under
that adapter's sandbox; `claude` needs no such grant under its auto mode.
...
The lifecycle below uses the shared schema-7 envelope and token-free recovery projection [[session-storage-2](session-storage.md#session-storage-2)] [[session-storage-7](session-storage.md#session-storage-7)] ([DR-049](../decisions/049-portable-session-contract.md)).
...
The canonical record shall use exactly the shared manifest fields and state variants [[session-storage-2](session-storage.md#session-storage-2)]; the recovery relationships below apply to its settled and uncertain variants, with the replay checkpoint published before the manifest [[session-storage-4](session-storage.md#session-storage-4)].
...
Where an uncertain marker carries a prior settled boundary and its authoritative ledger still equals the snapshot checkpoint, explicit discard shall be its sole rollback and shall restore the exact prior settled `updatedAt` and recovery fields while checkpointing the retained replay history; any ledger extension shall reject discard without mutation.
...
Ordinary continuation shall read current user config and explicit opening overlays, preserve the stored working directory and catalog, project to its stored playbooks and referenced players before validation or side effects, require every retained structural member to remain exact, and apply current model, effort, and optional fast mode to stored identities on the next call; uncertain retry shall instead use the attempt's exact stored projection.
...
A fully validated nonresumable record shall contribute its authenticated session id, lifecycle state, working directory, and update time only to predecessor ordering; a settled or history-only same-working-directory record shall decline adoption when it is the newest boundary, while an older or different-directory record shall not prevent adoption from a newer resumable predecessor.
...
Target Captain and player envelopes, model, effort, and fast-mode selections, player identities, descendant raw options, role-to-player bindings, effective commands, and intents shall not participate in that generation-envelope comparison.
...
Where either front end normalizes an overlaid inline agent block ([[playbook-cli-19](#playbook-cli-19)]), the shared launch path shall validate its adapter-scoped `effort`, any present literal `fastMode`, and every other agent field through the installed cligent loader before registry preparation or import.
...
The normalized model, effort, and fast mode shall reach both interactive and headless execution projections unchanged and may refresh from current config on an ordinary reopen; normalized instruction and permissions shall also reach both projections unchanged but shall remain structurally equal to the saved session; an uncertain retry shall retain every attempted setting exactly per [[playbook-cli-23](#playbook-cli-23)].
...
A replay stream or host-owned sidecar without that manifest shall not create a listable Playbook session, and unrecognized host files shall remain unchanged except for explicit validated migration or lease-bound deletion of the active legacy sidecar [[session-storage-10](session-storage.md#session-storage-10)] [[session-storage-12](session-storage.md#session-storage-12)].
The facade and both front ends shall use the same private store implementation and validators, so an external consumer applies the same validation the CLI applies rather than a copy of it, while both front ends and embedding hosts share the richer lifecycle [[session-storage-11](session-storage.md#session-storage-11)].
...
Where the writer prepares a record for the replay stream, it shall write a token-free projection: it shall strip every `resumeToken` field at any depth, every string-valued `resume` selection of [[playbook-runtime-34](playbook-runtime.md#playbook-runtime-34)], and every own enumerable object data member whose value is `undefined`, including those in schema-4 trace-event payloads carried by `captain_telemetry` records with topic `playbook.trace` under [[playbook-runtime-37](playbook-runtime.md#playbook-runtime-37)], while retaining `resume: false` as semantics rather than as a token.
An `undefined` array element shall remain unsanitizable rather than be changed to `null`; omitting an undefined-valued object member shall match the JSON projection of an optional observed-record member without changing another retained value.
In Captain/player `event` records and normalized adapter events (`type`, `agent`, finite `timestamp`, `payload`), it shall also remove provider `sessionId`, `nativeSessionId`, `threadId` and `conversationId` fields, including `ID` and `_id` spellings, and `id` inside provider `session`, `thread` and `conversation` metadata. This provider-ID rule applies recursively within provider metadata; it leaves tool `input` and `output` identifiers and Playbook logical session IDs unchanged.
Provider tokens shall remain solely in local checkpoint-bound hints [[session-storage-6](session-storage.md#session-storage-6)]; portable recovery uses the shared projection [[session-storage-7](session-storage.md#session-storage-7)], and replay alone remains insufficient to authorize continuation.
...
Each successful completed append shall advance `lastReadableSeq` without advancing `lastDurableSeq`, and the writer shall synchronize accumulated completed lines as one content checkpoint before each shared manifest checkpoint and before a normal lease release returns [[session-storage-4](session-storage.md#session-storage-4)]; only a successful checkpoint shall advance `lastDurableSeq` to `lastReadableSeq`.
For schema-7 sessions, checkpoint order, persisted incompleteness and continuation admission shall follow [[session-storage-4](session-storage.md#session-storage-4)]; the process-local status below never clears that durable marker.
...
| Case | Resolution |
| --- | --- |
| Key unset | `${SPEX_HOME:-$HOME/.spex}/sessions` [[session-storage-1](session-storage.md#session-storage-1)] |
...
The key shall never enter a persisted structural or execution projection ([[playbook-cli-23](#playbook-cli-23)]), and the changed default shall use the guarded former-default cutover [[session-storage-1](session-storage.md#session-storage-1)] [[session-storage-10](session-storage.md#session-storage-10)].

#### playbook-cli-92

When invoked as `playbook migrate-session <id> [--with <path>]...`, the CLI shall perform one explicit session migration:

- resolve the configured store with ordinary override precedence [[playbook-cli-78](#playbook-cli-78)];
- migrate the stopped legacy session through the shared source-retaining operation [[session-storage-10](session-storage.md#session-storage-10)], without registry imports, provider calls or a host launch;
- invalid arguments or migration refusal: exit `1` with a diagnostic;
- successful or already-completed migration: report the selected directory and resulting state, then exit `0`.
...
A successful host-disposal snapshot shall carry `lastSettlementStatus: ok` and `lastAction: runtime` for a leaf abandonment or `lastAction: dismiss` for a host-selected give-up [[playbook-captain-63](playbook-captain.md#playbook-captain-63)], with the same evidence and root-clear requirements at settlement and reread.
...
5. Serialize a detached provisional tmux-play config to an isolated explicit temporary path, validate and normalize it through the installed cligent `loadTmuxPlayConfig`, require cligent's public capability contract to accept every present Captain, player, or role fast-mode boolean, feed its Captain, player, layout, notification, and theme values back into the authoritative plan, and remove the temporary path without modifying the primary config or overlays.
...
- seeding twice in one invocation shall write one file and select one lineup.
...
When the test suite composes a top-level config enabling one or more playbooks, the test suite shall fail unless a dotted player id survives both interactive and headless projections byte-for-byte and an incapable host rejects it; the composed tmux-play config sets `captain.from` to `@sublang/playbook/playbook-captain`; carries exact `captain.options.sessionAgents` with one complete Captain block and exactly the referenced player blocks; carries one `captain.options.playbooks.<id>` entry per playbook with canonical prepared `from`, normalized effective `command`, exact closed role values `{ playerId, model, effort, fastMode? }` using the tagged tuning selections and optional effective boolean only where a role override or player default supplies one, and an option slice containing no launcher key; resolves scalar Captain and player values as adapter shorthands; normalizes full agent blocks; generates one roster entry per referenced player id; shares an entry only when role bindings name the same id; sets `layout.initialVisible` from the first enabled playbook with any bound players while leaving an all-roleless host-neutral plan empty; sets `captain.options.captainAdapter`; and carries normalized presentation fields (verifying [[playbook-cli-4](#playbook-cli-4)], [[playbook-cli-8](#playbook-cli-8)], [[playbook-cli-9](#playbook-cli-9)], [[playbook-cli-10](#playbook-cli-10)]).

### Validation

#### playbook-cli-15

When the test suite composes top-level configs carrying pre-import faults — a missing or blank `from`, top-level `profiles` or `run`, retired per-playbook `players`, an agent `profile`, an unknown root key, non-map `layout`, malformed or unknown agent fields, unknown adapter, conflicting or unsupported effort, malformed permissions, invalid presentation fields, invalid or reserved player or role ids, unknown bound players, role overrides outside model, effort, and fast mode, a non-boolean fast mode on a player or a role, either boolean fast mode supplied for an adapter that cligent reports unsupported, unresolved complete per-player tuning, authority-key smuggling, or non-JSON data — the suite shall fail unless each is rejected before registry preparation or import with a diagnostic naming the fault (verifying [[playbook-cli-4](#playbook-cli-4)], [[playbook-cli-8](#playbook-cli-8)], [[playbook-cli-9](#playbook-cli-9)], [[playbook-cli-28](#playbook-cli-28)], [[playbook-cli-33](#playbook-cli-33)], and [[playbook-cli-46](#playbook-cli-46)]).
...
The suite shall further fail unless a config binding the non-ASCII role keys `编码者` and `审查者` to declared players loads and covers a manifest requiring exactly those role ids, while an uppercase `Coder` role key, a role key carrying whitespace, and the reserved `captain` role key each reject before registry preparation or import (verifying [[playbook-cli-4](#playbook-cli-4)] and [[playbook-cli-9](#playbook-cli-9)]).
...
The suite shall further fail unless every turn exposes a durable uncertain baseline before its first effect; a failed or killed turn remains uncertain with stdout empty; ordinary continuation refuses it; retry reads no stdin, reuses its byte-exact input and attempted settings despite current config changes, advances the attempt before effects, and settles only once; discard restores the exact prior recovery baseline or deletes a compatible pre-turn-zero fresh record without config or host work; and SIGINT, SIGTERM, and SIGHUP cleanly retire ownership before the executable re-raises them (verifying [[playbook-cli-22](#playbook-cli-22)] and [[playbook-cli-23](#playbook-cli-23)]).
...
When the integration suite exercises the shared launch-config pipeline over temporary primary configs, overlays, synthetic schema-3 registries, legacy schema-2 rejection fixtures, and a selected stored projection and dry-packs the public package, it shall fail unless relative, absolute, file-URL, bare, and custom registry specifiers follow their defined resolution cases; an ordinary reopen drops every non-stored playbook and its additional players before validation or hooks, including malformed additions that would fail a fresh launch; installed-cligent normalization and capability validation of both present fast-mode booleans precede every retained preparation/import, with no Playbook-owned adapter support list; all retained preparation precedes every retained import; every consumed imported manifest member is read once before validation and projection; schema `2` and missing, malformed, or advertisement-disagreeing shared-factory and bespoke runtime profiles reject before host work; the prepared specifier, registry-authored manifest command, effective command, artifact schema `3`, required roles, concurrent role sets, referenced players, and exact role bindings reach interactive, headless, and stored projections; overlays and legacy-effort normalization leave source files unchanged; absent, literal-false, and literal-true fast mode reach both front ends and execution projections, ordinary reopen refreshes them, structural projection and retained-generation compatibility erase them, and uncertain retry preserves the exact attempted value; the plan is detached, deeply frozen, JSON-round-trippable, and free of imported functions and live capabilities; execution and presentation projections cannot mutate it; malformed retained host fields, an own `hostCapabilities` key, invalid identities or bindings, accessors, sparse arrays, symbols, undefined values, non-finite numbers, cycles, and non-plain objects are rejected before external hooks; readiness derives only from retained referenced agents; the leaf module has no host import and is re-exported compatibly; and the packed file list includes it (verifying [[playbook-cli-46](#playbook-cli-46)]).
...
Over a fresh working directory that is not a repository, the schema-3 host-capability suite shall fail unless that assembly succeeds with the capability identity, its write authority, and its observation naming that directory's prospective root at the null HEAD, a governed step that runs `git init` there classifies `unchanged`, and the following governed root commit classifies `one-descendant-commit` with that commit's OID on a boundary naming the same identity (verifying [[playbook-cli-20](#playbook-cli-20)]).
...
The recovery-control matrix shall fail unless exact-checkpoint discard preserves the previous settled recovery baseline, an extended ledger makes discard reject unchanged, settlement rejects a shell-ledger mismatch, ordinary continuation restores the exact settled ledger, and a failed reconstruction write leaves the record uncertain and starts no runtime or player (verifying [[playbook-cli-22](#playbook-cli-22)], [[playbook-cli-23](#playbook-cli-23)], and [[playbook-cli-63](#playbook-cli-63)]).
...
| Fixture and sanitizer | Recursive sanitization removes every `resumeToken`, string-valued `resume`, and own enumerable undefined-valued object member while preserving `resume: false`, and rejects an undefined array element; an opaque record with an unknown `type` and additional JSON-valued members round-trips unchanged; and the checked-in cross-repository fixture parses and rewrites to semantically equal envelopes with one terminating line feed per envelope regardless of JSON member order or source-checkout mode, including a role-omitting record whose role remains derivable from its trace ([[playbook-cli-74](#playbook-cli-74)], [[playbook-cli-75](#playbook-cli-75)]). |
...
When the integration suite runs the real CLI against a store containing application-created portable sessions, migrated desktop history and unmigrated legacy sessions, it shall verify the shared storage flow:

- compatible continuation among skipped nonresumable histories [[playbook-cli-22](#playbook-cli-22)];
- fresh creation [[playbook-cli-23](#playbook-cli-23)];
- explicit migration through configured overrides without provider work [[playbook-cli-92](#playbook-cli-92)];
- continued headless and interactive launches refuse invalid replay, required context and path boundaries before module import or host work [[playbook-cli-22](#playbook-cli-22)] [[playbook-cli-49](#playbook-cli-49)].
