...
Validation, stale-selection, or ownership failure before source publication shall leave both records unchanged; the guarded exchange shall publish the source clear before the target install, hold both leases through its final checks, never compensate after target publication may have occurred, and report success only after the target install and both post-write owner checks, so no failure boundary leaves the same generation transferable from both records.
