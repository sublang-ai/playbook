...
The key shall never enter a persisted structural or execution projection ([[playbook-cli-23](#playbook-cli-23)]), and the changed default shall use the explicit migration boundary rather than silently leaving the old default's sessions behind [[session-storage-10](session-storage.md#session-storage-10)].
