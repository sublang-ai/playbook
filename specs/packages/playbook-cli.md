...
The command shall use current normalized model, effort, and optional fast mode for the stored identities on the next call and shall fail without a fresh fallback when the provider rejects those settings on the stored token.
