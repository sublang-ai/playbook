...
A record schema `3` or historical schema `4` shall receive no inferred migration and shall reject before registry construction or governed work because its snapshot cannot prove artifact-schema-3 effect authority.
When implicit `--continue` scans the durable store, it shall report and skip each fully validated released record schema `2`, `3`, or `4` by exact session id and path with the same applicable incompatibility or effect-authority-cutover reason as explicit selection and an archive-or-remove remedy; canonical schema `5` shall remain selectable, while malformed or unsafe records and unknown schemas remain fail-closed.
...
The legacy-record matrix shall fail unless fully validated record schemas `2`, `3`, and `4` are nonresumable and receive no migration, implicit selection reports and skips them beside a valid schema-5 record, explicit selection rejects them before registry import or host work, and a canonical schema-5 record carrying artifact schema `2`, a direct pre-ledger shell or runtime snapshot, malformed ledger data, and an unknown schema each fail closed (verifying [[playbook-cli-22](#playbook-cli-22)] and [[playbook-cli-23](#playbook-cli-23)]).
