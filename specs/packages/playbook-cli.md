...
A record schema `3`, historical schema `4`, or otherwise valid transitional schema-`5` record lacking required `unresolvedEffects` shall receive no inferred migration and shall reject before registry construction or governed work because it cannot prove artifact-schema-3 effect authority and its complete settlement evidence.
When implicit `--continue` scans the durable store, it shall report and skip each fully validated released record schema `2`, `3`, or `4` and each recognized transitional schema-`5` record by exact session id and path with the same applicable incompatibility or effect-authority-cutover reason as explicit selection and an archive-or-remove remedy; complete canonical schema `5` shall remain selectable, while other malformed or unsafe records and unknown schemas remain fail-closed.
...
Record schemas `3` and `4` and an otherwise valid transitional schema-`5` record lacking required `unresolvedEffects` shall receive no migration; such a fully validated record may be classified as nonresumable for implicit selection or unrelated fresh discovery, while explicit selection and every direct pre-ledger shell or runtime snapshot shall reject before registry import, runtime construction, source-state restoration, or governed work rather than fabricate missing authority.
...
Every schema-version-5 Captain record shall carry required `unresolvedEffects` validated through the pure public boundary of [[playbook-captain-58](playbook-captain.md#playbook-captain-58)]; a fresh boundary shall use the canonical empty list, a settled record shall carry the shell settlement's exact detached copy, and uncertain marking, retry, authoritative reread, and exact discard shall preserve the prior settled copy byte-for-byte until a later safe settlement replaces it.
...
The legacy-record matrix shall fail unless fully validated record schemas `2`, `3`, and `4` and an otherwise valid transitional schema-`5` record lacking `unresolvedEffects` are nonresumable and receive no migration, implicit selection and unrelated fresh discovery report and skip them beside a complete schema-5 record, explicit selection rejects them before registry import or host work with the applicable explanation, and a complete canonical schema-5 record carrying artifact schema `2`, a direct pre-ledger shell or runtime snapshot, malformed ledger data, and an unknown schema each fail closed (verifying [[playbook-cli-22](#playbook-cli-22)] and [[playbook-cli-23](#playbook-cli-23)]).
