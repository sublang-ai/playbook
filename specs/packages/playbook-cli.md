...
In Captain/player `event` records and normalized adapter events (`type`, `agent`, finite `timestamp`, `payload`), it shall also remove provider `sessionId`, `threadId` and `conversationId` fields, including `ID` and `_id` spellings, and `id` inside provider `session`, `thread` and `conversation` metadata. This provider-ID rule applies recursively within provider metadata; it leaves tool `input` and `output` identifiers and Playbook logical session IDs unchanged.
