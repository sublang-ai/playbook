...
When implicit `--continue` scans the durable store, it shall report and skip each fully validated released record schema `2`, `3`, or `4` by exact session id and path with an archive-or-remove remedy; canonical schema `5` shall remain selectable, while malformed or unsafe records and unknown schemas remain fail-closed.
