...
An explicitly selected released Captain-session record schema `2`, shell snapshot schema `1`, or runtime snapshot schema `1` or `2` shall receive an incompatible-player-identity diagnostic rather than an inferred conversion to schema version `3`.
When implicit `--continue` scans the durable store, it shall report and skip each released Captain-session record schema `2` by exact session id and path with an archive-or-remove remedy; malformed or unsafe records and unknown schemas shall remain fail-closed.
...
The suite shall further fail unless the compiled session Captain's own factory construction rejecting a compatibility declaration during host construction produces that same setup diagnostic and exit `1` with nothing on stdout, and unless the compiled Captain module defers its factory call to the first runtime request, since the front ends import that one module statically and an eager call would surface the rejection as an uncaught module-load error instead (verifying [[playbook-cli-20](#playbook-cli-20)]).
