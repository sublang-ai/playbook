...
Where an append call has passed the argument boundary of [[playbook-cli-73](#playbook-cli-73)] on a replay writer whose numeric live status has `incomplete: false` under [[playbook-cli-83](#playbook-cli-83)], the writer shall place it behind every earlier admitted call and serialize its sanitizer and append work through promise settlement in JavaScript invocation order, assigning the next sequence one past the then-current `lastReadableSeq` only when the call reaches the queue head.
