...
After safe host cleanup, a headless launch shall retract its own byte-exact empty turn-zero target when a later pre-marker failure aborts launch, while a managed launch shall retract only by winning the child-shutdown claim before the accepted-readiness witness of [[playbook-cli-49](#playbook-cli-49)]; either front end shall preserve a changed, nonempty, or ambiguously owned target.
