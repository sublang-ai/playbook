...
Each retained generation shall contain exactly one nonempty root-to-leaf frame stack of the shell's frame shape ([[playbook-captain-41](playbook-captain.md#playbook-captain-41)]), with each frame carrying its playbook and engagement identities, depth, options, role bindings, and active quiescent schema-4 runtime snapshot exported under [[playbook-runtime-45](playbook-runtime.md#playbook-runtime-45)]; the root shall match the map key, every frame shall name the stored catalog and required role set, every parent suspended-call descriptor shall exactly match its child edge, and the leaf shall be parked without a suspended child.
...
A successful exchange shall move the complete detached retention map into the target's first record and clear the complete source map, without transferring a compatible subset or changing either record's snapshot, effect ledger, projections, lifecycle state, generation contents, or unrelated members except `updatedAt`; both publications shall use canonical schema version `5`, source clear shall advance its prior timestamp, and target install shall advance strictly after both its proposed timestamp and the source clear so the new owner is the next deterministic predecessor under [[playbook-cli-23](#playbook-cli-23)].
...
The writer shall require a record to be uncertain before every ledger mutation, shall preserve the pre-turn shell ledger as its immutable checkpoint, and shall keep every write a monotonic extension of that checkpoint.
...
Recovery shall preserve boundary, operation, attempt, original-baseline, checkpoint, semantic, question, continuation, eligibility, and spent-budget data exactly, and a failed or indeterminate recovery write shall keep the record uncertain, start no runtime or player, and quarantine the applicable claim.
...
When the persistence integration suite exercises retained generations through real record files, the real Captain shell, both front-end settlement callers, and successive leases, the suite shall fail unless a nested root-to-leaf stack, its optional nonblank root-state description, both call-bridge halves, and its runtime ledger mirrors round-trip as one detached unit across settlement, host and runtime disposal, reread, uncertain retry, and exact discard; a generation omitting that description remains valid; a member-less schema-5 record remains selectable and restores byte-exactly after discard; schema-2-only records `3` and `4` migrate at their next mutation; a later settlement replaces only its named root; shell-derived clean completion clears only its named root; malformed, final, nonquiescent, cyclic, duplicate, unknown, dangling, or ledger-inconsistent stacks and duplicate or malformed updates reject without settlement; and pre-rename or post-rename durability faults expose only a complete old or new snapshot-generation-ledger unit (verifying [[playbook-cli-23](#playbook-cli-23)] and [[playbook-cli-51](#playbook-cli-51)]).
...
The suite shall further fail unless a nested multi-root map moves whole and detached from one settled predecessor into an absent fresh target while preserving every generation identity and unrelated record member, advances source then target monotonically under a tied or regressed clock, leaves the target as the selected predecessor, and restores that target boundary exactly after a later uncertain marker is discarded; guarded initialization declines every exercised root-envelope mismatch and missing descendant catalog with the source byte-exact and an empty target, every permitted Captain, player, tuning, descendant-option, role-binding, command, and intent difference remains accepted, target reuse or owner loss rejects under authoritative reread, predecessor disappearance or supersession declines with an empty newest target, and two contenders transfer at most one copy (verifying [[playbook-cli-51](#playbook-cli-51)] and [[playbook-cli-53](#playbook-cli-53)]).
