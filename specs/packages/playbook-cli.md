...
Before invoking managed attachment, the outer command shall retain SIGHUP, SIGINT, and SIGTERM handlers and pass their first recorded signal through the prepared attachment's activation-abort boundary; when interruption precedes native-client hand-off, it shall await child shutdown, acknowledgement, pane exit, and coordination cleanup before re-raising that same signal.
When the prepared attachment synchronously invokes the native-client hand-off callback, the outer command shall remove its handlers and mark ownership transferred before the inherited-terminal tmux client starts; a later client-side signal shall use native detach or termination semantics and shall not request child shutdown or lease retirement.
Where native attachment is disabled, the outer command shall retain its handlers through activation and coordination cleanup, remove them after success, and use the same cleanup-before-re-raise order for an interrupted activation.
