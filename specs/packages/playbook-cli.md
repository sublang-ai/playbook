...
An explicitly selected released Captain-session record schema `2` shall receive an incompatible-player-identity diagnostic rather than an inferred conversion to record schema version `5`.
...
A record schema `3` or historical schema `4` whose complete structural catalog contains only artifact schema `2` shall receive the explicit pre-effect migration of [[playbook-cli-23](#playbook-cli-23)], while either record schema with any artifact schema `3` shall reject before registry construction or governed work because its snapshot cannot prove an effect ledger.
When implicit `--continue` scans the durable store, it shall report and skip each released record schema `2` or pre-ledger schema-3-bearing record by exact session id and path with an archive-or-remove remedy; migratable records and canonical schema `5` shall remain selectable, while malformed or unsafe records and unknown schemas remain fail-closed.
...
After the selected configured-option and working-directory compatibility checks succeed under the current logical-session lease, a headless host with any schema-3 catalog member shall first require that lease's session id to equal the selected logical id, then resolve that working directory's canonical Git identity once, require the lease-owned atomic effect-ledger writer and synchronous detached mirror of [[playbook-cli-63](#playbook-cli-63)], and assemble one frozen artifact-specific capability per schema-3 id from the current session id and owner token, detached manifest roles and cohorts, bound repository observer and coordinator, mirror, and writer; an absent writer or mismatched lease shall reject before shell construction, while a schema-2-only catalog shall touch none of those Git, lease-assertion, or writer dependencies ([[playbook-runtime-50](playbook-runtime.md#playbook-runtime-50)], [[playbook-runtime-67](playbook-runtime.md#playbook-runtime-67)], [[playbook-cli-23](#playbook-cli-23)], [[playbook-cli-57](#playbook-cli-57)]).
...
The canonical record shall be a closed, strictly plain-JSON Captain-session schema version `5`, distinct from the released direct-run-only, root-owned-player, and pre-effect-ledger boundaries.
A settled record shall contain exactly `schemaVersion`, `kind`, `state`, `sessionId`, `createdAt`, `updatedAt`, `cwd`, `structuralProjection`, `lastAppliedExecutionProjection`, `snapshot`, and required `effectLedger`, with the optional `retainedGenerations` member; an uncertain record shall contain those same common and optional fields plus exactly one `uncertain` member.
...
A record schema `3` or historical schema `4` shall be migrated only when its complete structural catalog contains exclusively artifact schema `2`: the pure migration shall recursively advance every runtime and shell snapshot from schema `3` to `4`, install the canonical empty effect ledger at every level and record root, advance the record to schema `5`, preserve every other value, and remain in memory until the next lease-owned atomic mutation.
Either legacy record with an artifact schema `3`, and every direct pre-ledger shell or runtime snapshot, shall reject before registry import, runtime construction, source-state restoration, or governed work rather than fabricate missing authority.
...
The store shall require the record's authoritative ledger to be an exact or monotonic extension of every generation checkpoint; an unmarked generation's schema-3 frame mirrors shall equal that checkpoint and its schema-2 mirrors shall be empty, while a marked generation shall preserve its original checkpoint and source generation and carry one common capture-time authoritative mirror that strictly extends the checkpoint, remains an exact or monotonic baseline of the record ledger, and has matching runtime markers in every schema-3 frame under [[playbook-captain-54](playbook-captain.md#playbook-captain-54)].
...
A canonical schema-version-5 record may omit `retainedGenerations`; the store shall interpret absence as an empty map for explicit selection, latest-session selection, ordinary continuation, and retention updates while preserving the absent member through uncertain marking, retry, and exact discard.

#### playbook-cli-53

Where a lease owns an absent target id and has validated its proposed fresh settled turn-zero Captain-session boundary with an empty retention map under [[playbook-cli-51](#playbook-cli-51)], when the store selects its cross-session predecessor, the store shall choose the newest other settled compatible record with the same normalized working directory by canonical `updatedAt` and the deterministic session-id tie-break of [[playbook-cli-23](#playbook-cli-23)]; uncertain candidates and different-directory records shall be ineligible, while a malformed, unsafe, or unknown-schema canonical record shall fail closed rather than be skipped.
...
A successful exchange shall move the complete detached retention map and the source record's complete authoritative effect ledger into the target's first record, clear only the complete source map while preserving the source ledger, and rebase only the target turn-zero shell's ledger mirror to that transferred ledger while keeping its internal Captain mirror empty; it shall transfer no compatible subset and shall change neither record's projections or lifecycle state nor any generation checkpoint, frame, marker, or unrelated member except `updatedAt`, and the source snapshot shall remain unchanged.
...
Both publications shall use canonical schema version `5`, source clear shall advance its prior timestamp, and target install shall advance strictly after both its proposed timestamp and the source clear so the new owner is the next deterministic predecessor under [[playbook-cli-23](#playbook-cli-23)].
...
Where the current process owns a schema-version-5 Captain record's exact session lease, the store shall expose one private `writeEffectLedger(authority, commands)` that validates the artifact schema, logical session, lease-owner token, canonical worktree, and current schema-3 catalog membership before each atomic mutation and shall persist no authority or live handle ([[playbook-runtime-69](playbook-runtime.md#playbook-runtime-69)]).
...
At settlement the store shall require exact equality among the authoritative ledger, shell schema-version-4 mirror, and every schema-3 frame mirror; ordinary continuation shall restore that same value, while schema-2 and internal Captain mirrors remain empty ([[playbook-captain-50](playbook-captain.md#playbook-captain-50)]).

#### playbook-cli-65

Where the current process owns a schema-version-5 Captain record and services the deferred Boss-question operation of [[playbook-runtime-73](playbook-runtime.md#playbook-runtime-73)], the private host capability shall hold the exclusive canonical-worktree claim while it compares a saved checkpoint, starts or completes a physical boundary, binds or replaces the logical operation, marks checkpoint-restoration eligibility, or consumes that eligibility.
...
Where a schema-version-5 Captain record retains a generation, the store shall validate the generation's required complete capture-time effect-ledger checkpoint and optional retained-effect marker together with its frame snapshots, require the record's authoritative ledger to be an exact or monotonic extension of that checkpoint, require every ordinary schema-3 frame mirror to equal the checkpoint and every schema-2 mirror to be empty, and require every marked schema-3 frame to carry one common capture-time authoritative mirror that strictly extends the checkpoint, remains an exact or monotonic baseline of the record's current ledger, and has matching source-lineage and checkpoint evidence under [[playbook-captain-54](playbook-captain.md#playbook-captain-54)]; a generation-level marker shall remain valid without a frame marker where its retained stack contains no schema-3 frame.
The guarded predecessor exchange of [[playbook-cli-53](#playbook-cli-53)] shall preserve the source record's authoritative ledger while atomically moving its complete retention map, install that ledger as the target record and turn-zero shell mirror, keep the target internal-Captain mirror empty, and preserve each generation and capture checkpoint byte-for-byte; the target catalog shall supply compatible schema-3 authority for every ledger boundary and logical operation before either transfer publication.
The source clear, target install, rollback, publication-ambiguity, owner-check, no-subset, and single-transfer rules shall apply to the ledger-map-snapshot unit, so every visible target is either the intentional empty boundary or one complete target whose authoritative ledger, shell mirror, and retained checkpoints are mutually lawful, and no failure shall erase the source ledger or publish an effect-free target around a transferred generation.
...
When the persistence integration suite exercises retained generations through real record files, the real Captain shell, both front-end settlement callers, and successive leases, the suite shall fail unless a nested root-to-leaf stack, its required capture-time effect checkpoint, optional retained-effect marker and nonblank root-state description, both call-bridge halves, and its runtime ledger mirrors round-trip as one detached unit across settlement, host and runtime disposal, reread, uncertain retry, and exact discard; a generation omitting either optional member remains valid; a member-less schema-5 record remains selectable and restores byte-exactly after discard; schema-2-only records `3` and `4` migrate at their next mutation; a later settlement replaces only its named root; shell-derived clean completion clears only its named root; malformed, final, nonquiescent, cyclic, duplicate, unknown, dangling, or ledger-inconsistent stacks and duplicate or malformed updates reject without settlement; and pre-rename or post-rename durability faults expose only a complete old or new snapshot-generation-ledger unit (verifying [[playbook-cli-23](#playbook-cli-23)] and [[playbook-cli-51](#playbook-cli-51)]).

#### playbook-cli-54

When the persistence integration suite selects cross-session predecessors through real record files and successive exclusive leases, the suite shall fail unless settled, uncertain, different-directory, deterministic-tie, malformed, unknown-schema, member-less, explicitly empty, and live-leased candidates prove exact newest-predecessor selection; an empty or live newest predecessor shall never fall through to an older nonempty map, the live case shall publish an empty fresh target without changing either source, and foreign or permission-unknown ownership shall still fail closed (verifying [[playbook-cli-23](#playbook-cli-23)] and [[playbook-cli-53](#playbook-cli-53)]).
...
For the authorized mixed-schema replay, the suite shall fail unless the stored pre-turn snapshot and complete authoritative suffix remain intact, only the detached restore copy's shell-root and active schema-3 frame mirrors are rebased, schema-2 and internal-Captain mirrors remain empty, the attempt increments once before one Boss-turn replay using the exact saved input and attempted config, and settlement retains the complete ledger (verifying [[playbook-cli-67](#playbook-cli-67)]).

#### playbook-cli-70

When the retained-generation persistence suite settles exact and fenced generations, it shall fail unless every stored generation carries a valid complete capture-time checkpoint, the record ledger is its exact or monotonic extension, ordinary schema-3 frame mirrors equal that checkpoint, every fenced schema-3 frame preserves one byte-identical capture-time mirror that strictly extends the checkpoint and remains an exact or monotonic baseline of a later record ledger, schema-2 mirrors stay empty, a schema-2-only fenced generation remains valid, and the pure record validator rejects an incomplete checkpoint or mutation of the checkpoint, source marker, frame marker, shared capture mirror, ledger authority, or monotonic relationship before persistence (verifying [[playbook-cli-69](#playbook-cli-69)]).
