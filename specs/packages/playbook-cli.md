...
A record schema `3` or historical schema `4` shall be migrated only when its complete structural catalog contains exclusively artifact schema `2`: the pure migration shall recursively advance every runtime and shell snapshot from schema `3` to `4`, install the canonical empty effect ledger at every level and record root plus canonical empty `unresolvedEffects`, advance the record to schema `5`, preserve every other value, and remain in memory until the next lease-owned atomic mutation.
...
Every schema-version-5 Captain record shall carry required `unresolvedEffects` validated through the pure public boundary of [[playbook-captain-58](playbook-captain.md#playbook-captain-58)]; a fresh or schema-2-only migrated boundary shall use the canonical empty list, a settled record shall carry the shell settlement's exact detached copy, and uncertain marking, retry, authoritative reread, and exact discard shall preserve the prior settled copy byte-for-byte until a later safe settlement replaces it.
