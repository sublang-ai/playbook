...
After those authoritative compatibility checks and under that same child-owned lease, the managed host shall require a host-supplied write-ahead channel, assemble the same current artifact-specific schema-3 repository and atomic-ledger capabilities as the headless host under [[playbook-cli-20](#playbook-cli-20)], and pass them only through the shared shell construction boundary; absent writer authority, a logical-session mismatch, or lost lease ownership shall fail before shell initialization or restoration and before readiness.
