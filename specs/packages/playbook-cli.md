...
A lease-free stream read shall validate a fresh filesystem snapshot on every call, return the complete readable prefix of [[playbook-cli-74](#playbook-cli-74)] or its requested incremental suffix with `lastReadableSeq` equal to the final sequence of the whole prefix or `0` when absent or empty, expose no `lastDurableSeq` or `incomplete` member, and mutate or repair nothing; those writer-live facts are not observable across processes.
After acquiring the valid exclusive session lease of [[playbook-cli-23](#playbook-cli-23)], the replay writer shall rescan and validate the complete retained prefix, seed both `lastReadableSeq` and `lastDurableSeq` from its final sequence or `0`, and begin with `incomplete: false`, even when that store previously followed the stream without a lease.
If that replay-only initialization cannot validate the stream file boundary or complete prefix, the already-acquired canonical lease shall remain usable while the replay writer enters the exact live status `{lastReadableSeq: null, lastDurableSeq: null, incomplete: true}`, performs no replay append or checkpoint, leaves the stream byte-identical, and retains the strict read rejection of [[playbook-cli-74](#playbook-cli-74)] rather than returning a partial history.
The live status shall be exactly the union of `{lastReadableSeq: <nonnegative integer>, lastDurableSeq: <nonnegative integer>, incomplete: <boolean>}` after successful initialization and that null-boundary variant after failed initialization; both sequence fields shall be null together if and only if initialization established no trustworthy whole-stream boundary.
Where a host appends through an initialized replay writer, appends shall be awaited in record order with the next sequence one past `lastReadableSeq`; an unavailable writer shall suppress every append.
...
When replay initialization, an append, first-publication directory synchronization, or content checkpoint fails, the writer shall retain or set the corresponding `incomplete: true` status and stop appending for the remainder of the session, but shall neither fail nor undo the agent turn, a successful private session-record settlement, or an otherwise valid lease release; this replay-only isolation begins after the canonical directory and lease have passed the fail-closed boundary of [[playbook-cli-23](#playbook-cli-23)].
After successful initialization, an append, publication, or checkpoint failure shall leave the numeric `lastDurableSeq` unchanged and keep the readable file a clean contiguous prefix.
...
A lease whose replay initialization failed shall retain strict stream-read rejection while status reports the null-boundary incomplete variant, and normal release shall return that variant without letting replay failure prevent canonical lease retirement; otherwise normal release shall return final numeric status after its checkpoint attempt.
A successor lease shall rescan whichever complete prefix the filesystem retained; where it validates, the lease shall continue one past its `lastReadableSeq`, seed both numeric sequence values from that prefix, and report no inherited latch, while an unchanged invalid stream shall independently produce the unavailable status again.
Repairing a torn final line shall be lawful only under a held lease.
...
Teeing shall change neither the turn outcome nor the buffered stdout and stderr contract of [[playbook-cli-18](#playbook-cli-18)] and [[playbook-cli-20](#playbook-cli-20)].
...
| Checkpoint progression | Acquiring against an absent or empty stream initializes both reported sequence values to `0`, acquiring against an existing valid complete prefix seeds both to its final sequence, each facade append advances only `lastReadableSeq`, a successful private settlement checkpoint advances `lastDurableSeq` to it, later appends again advance only readability, and a normal facade release directly proves its pending content synchronization before returning final status with both values equal and `incomplete: false` ([[playbook-cli-76](#playbook-cli-76)]). |
| Independent follower | Before and after another store's lease appends a complete but uncheckpointed line, an independent lease-free reader rescans the file, returns exactly the complete prefix or requested suffix, reports `lastReadableSeq` equal to the whole prefix's final sequence, and exposes no durable-sequence or incomplete-state member; acquiring the lease after an earlier follower read rescans the then-current prefix rather than seeding from stale follower state ([[playbook-cli-76](#playbook-cli-76)]). |
