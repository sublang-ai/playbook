...
| `PR-6` | merge | capture the inferred pull-request URL and repository default branch; `gh pr merge --merge --delete-branch --match-head-commit "$(git rev-parse HEAD)"`; require that same pull request to report `MERGED` and the local checkout to name the default branch | merged | merge refused |
