...
The suite shall further fail unless an authored mid-workflow checkpoint — parked, no pending question, not one of the three entries, like the acceptance checklist fixture's `outline` — classifies only its nonempty text under its own Boss-event contracts, while empty or whitespace-only input there produces the same trace-only no-action as every state: no event, judge call, player call, status emission, or FSM transition (verifying [[playbook-runtime-7](#playbook-runtime-7)]).
