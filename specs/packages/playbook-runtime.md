...
The current CODE, REVIEW, DECIDE, DEV, BRANCH, and PR registries accept no workflow-specific options and shall reject every nonempty option slice.
...
The supported set shall be exactly `[3]` under runtime ABI `1` and shall exclude every legacy artifact schema.
...
Each schema-3 outcome shall declare exactly one repository disposition from `unchanged`, `one-descendant-commit`, or `deferred`; effect-owned fields shall occur only on `one-descendant-commit` or `unchanged` ([DR-045](../decisions/045-unchanged-receipt-revision-authority.md)), and `deferred` shall occur only on `needsBossReply` with a presentation-owned question and a sibling `one-descendant-commit` outcome.
The linker shall derive that disposition from the source-derived operation and the outcome's required repository effect independently of payload-field presence; a commit-required completion shall declare `one-descendant-commit` even for a generic `done` arm with `fields: {}`, and absence of an effect-owned field shall not justify `unchanged`.
...
- `one-descendant-commit`, carrying the after OID, only when after HEAD is exactly one commit descended from baseline HEAD — every commit descends from the null HEAD, so a first root commit qualifies — and every baseline projection entry is preserved, absorbed by that commit, or altered and carried by it, with no other entry in the after projection ([DR-062](../decisions/062-pre-existing-changes-are-context.md));
...
- `worktree-only-change` for a same-HEAD projection delta from an effect-authorized call only when every baseline projection entry is preserved or altered, none lost;
- `concurrent-or-foreign-change` for any delta from a call declared exclusively `unchanged`; and
- `observation-ambiguous` for an unstable or lossy observation, a baseline entry lost, a baseline entry altered but left uncommitted beside a commit, post-commit residual, cohort overlap, or other delta that cannot be attributed uniquely.

A baseline projection entry is *preserved* when the after projection holds its byte-equal entry, *absorbed* when it is gone from the after projection and the after HEAD tree carries the content the baseline recorded for that path — the worktree content of a modified or untracked entry, the index content of a staged entry, the absence of a deleted one — *altered* when it stands in the after projection as a different entry or the commit carries the path with other content, and *lost* when it is gone from the after projection and the commit does not carry the path's change; a rename, an unmerged path, a submodule, or a nested worktree is preserved when byte-equal and otherwise lost, and content is compared by the projection's own content addressing, never by Git object id alone.
Receipt classification shall compare the complete projections and ancestry without a post-hoc path filter, shall treat byte- and mode-identical pre-existing dirt as zero delta, shall record the absorbed, altered, and lost paths on the receipt [[playbook-runtime-69](#playbook-runtime-69)], and shall not claim that cooperative observation excludes or identifies every foreign writer.
...
The first or a repeated question shall become pending, emit status, or enter an exportable wait only after its physical boundary has a complete same-HEAD after observation with neither rewritten or multiple-commit history nor concurrent, foreign, or ambiguous evidence, and while the same-worktree claim remains held the host has atomically bound that boundary to the operation's stable UUID, immutable original baseline, full physical-order boundary chain, latest after checkpoint, exact pending question, and exact token-free player descriptor `{v:1,playerId}`, with provider continuity selected separately from the current player store when the call starts [[session-storage-7](session-storage.md#session-storage-7)].
...
Once that start is acknowledged and the runtime sends the authored continuation event, the classification status, that transition, and every later transition, accepted outcome, and status of the continuation shall publish in causal order under [[playbook-runtime-37](#playbook-runtime-37)] — ahead of the continued player call and of any nested playbook call a target state starts — while an exit that starts no player publishes none of them.
...
Where an artifact-schema-3 governed outcome supplies a presentation-owned question, declares `unchanged`, and enters the authored Boss-reply wait, the runtime shall publish the question only after its exact matching physical receipt is acknowledged under [[playbook-runtime-77](#playbook-runtime-77)], shall create no logical operation and therefore no operation-owned checkpoint, pending-question copy, player-continuation copy, or restoration eligibility, and shall route a later valid answer under [[playbook-runtime-7](#playbook-runtime-7)] through the ordinary authored Boss-reply continuation as a new separately governed physical boundary.
Where that shared-factory wait [[playbook-runtime-45](#playbook-runtime-45)] resumes a state that also declares a `deferred` `needsBossReply` arm, the runtime shall retain the deferred-binding requirement unless its current authoritative ledger [[playbook-runtime-69](#playbook-runtime-69)] has no open logical operation owned by the runtime or its retained source lineage and its latest physical boundary owned by either identity matches the pending resume state and role, has no logical-operation identity, carries a complete `unchanged` receipt, and revalidates under [[playbook-runtime-77](#playbook-runtime-77)] as a resolved `unchanged` outcome whose question equals the exact pending text.
The runtime shall neither substitute an older matching boundary for that latest owned boundary nor infer the selected disposition from the state's outcome menu, and missing, mismatched, or unresolved origin evidence shall preserve the existing deferred protections across live continuation and restore.
Nongoverned Boss-question paths shall retain their existing continuation behavior.
...
When no Boss question is pending, the current CODE, REVIEW, DECIDE, DEV, BRANCH, and PR runtimes shall use their deterministic initial event under [[playbook-runtime-1](#playbook-runtime-1)].
...
When the runtime starts an effect-authorized player call whose baseline projection [[playbook-runtime-67](#playbook-runtime-67)] is nonempty, the call prompt shall end with a pre-existing-changes block ([DR-062](../decisions/062-pre-existing-changes-are-context.md)): the baseline's paths grouped as staged, modified, deleted, untracked, renamed, unmerged, and other, at most 40 paths in all with the count of any remainder, followed by the rule that they existed before this call and belong to the Boss, are left exactly as they are unless the task or the Boss's request requires building on them, are never reverted or deleted, and are named in the final report when committed:

- the block is composed from the call's own baseline observation, so the prompt the trace records is the prompt sent;
- an empty baseline projection adds nothing, and a call declared exclusively `unchanged` receives no block.
...
A governed delegated-player call shall validate and assemble that reply under [[playbook-runtime-77](#playbook-runtime-77)].
...
For a nongoverned delegated-player call, a reply that is malformed, names an undeclared guard, or omits a required extracted non-verbatim field shall cause the runtime to throw; a governed delegated-player call shall follow the bounded correction and parking contract of [[playbook-runtime-77](#playbook-runtime-77)].
...
Where a factory-backed artifact supplies an artifact-specific status override, the runtime shall still require complete local-role `roleStates` metadata; missing metadata shall reject at construction instead of selecting metadata-absent legacy status defaults.
...
During shell initialization, each CODE, REVIEW, DECIDE, DEV, BRANCH, and PR registry shall accept an absent or empty object as its option slice and shall reject a non-object or any unknown key with a diagnostic naming `captain.options.playbooks.<id>.options` and the offending key when present.
...
A rejected call carrying no result shall preserve the prior token and shall not trigger a runtime-level fresh retry; the shared host alone handles a classified pre-execution session rejection within the same logical call [[session-storage-8](session-storage.md#session-storage-8)].
...
current detached `session.roleBindings`, require the snapshot's complete effect ledger to equal the current capability mirror, restore the local-role token projection and the failure-attempt meaning of [[playbook-runtime-71](#playbook-runtime-71)],
...
- A factory-backed fixture with complete `roleStates` and no status override emits the bare classification before dispatch, only metadata-backed `⤷ <Role>: <label>` entries, exact `→ <acceptedOutcome>` from confirmed evidence with no tally, rider, or leading whitespace, the compact-data failure marker, and both exact Boss-wait lines, while idle, terminal, and unlisted states produce no canonical fallback (verifying [[playbook-runtime-3](#playbook-runtime-3)] and [[playbook-runtime-14](#playbook-runtime-14)]).
- A schema-3 factory-backed fixture with missing or incomplete `roleStates` rejects during construction even when a status override exists, before machine interpretation or a host call (verifying [[playbook-runtime-57](#playbook-runtime-57)]).

#### playbook-runtime-68

When the repository-observation integration suite drives real temporary Git worktrees, it shall fail unless staged, tracked-worktree, mode-only, non-ignored-untracked, and nested-worktree dirty-content changes alter the detached content-addressed projection while ignored-only and timestamp-only changes do not; unchanged pre-existing dirt proves `unchanged`; exactly one call-created descendant commit that preserves the complete baseline projection proves `one-descendant-commit` with the exact OID, as does a worktree's first root commit from a baseline whose unborn HEAD observed as the null OID; one commit that absorbs a modified, untracked, staged, or deleted pre-existing entry, or alters one and carries it, proves `one-descendant-commit` with that path recorded as absorbed or altered while an untouched neighbour stays unrecorded; an altered pre-existing entry left uncommitted at the same HEAD proves `worktree-only-change` with the path recorded as altered; a pre-existing entry reverted or deleted by the call is recorded as lost and stays fail-closed; and residual changes beside a commit, multiple commits, rewritten or non-descendant history, declared-zero deltas, inspection-suppressing index flags, unreadable reported content, non-lossless path data, and mutation during observation receive their exact fail-closed classifications without a path-attribution filter (verifying [[playbook-runtime-67](#playbook-runtime-67)]).
...
The snapshot matrix shall prove that every linked workflow runtime frame receives and exports the complete current-host mirror while the internal Captain runtime receives and exports the canonical empty ledger; restore shall reject a mirror mismatch and adoption shall reject any mismatch not admitted by the retained-ledger fence before actor, store, port, trace, status, or telemetry work (verifying [[playbook-runtime-45](#playbook-runtime-45)] and [[playbook-runtime-69](#playbook-runtime-69)]).
...
When the deferred-continuation integration matrix drives a governed artifact-schema-3 question chain through real observation, coordination, and an acknowledged effect-ledger capability, it shall fail unless the initial question remains unpublished until its boundary receipt and bound logical operation are durable; the operation carries the exact original baseline, first checkpoint, pending question, token-free `{v:1,playerId}` binding, and reciprocal physical link; and an exact-checkpoint valid answer atomically consumes that wait and persists its next started boundary before exactly one player call (verifying [[playbook-runtime-73](#playbook-runtime-73)]).
The matrix shall fail unless a restored bound operation selects the current player store token or `false` without persisting a provider token or changing its operation identity or original baseline, while a mismatched player binding rejects before a new boundary or call.
...
The matrix shall fail unless, once a valid answer starts its continuation, the trace sequences the classification status and the authored transition before the continued player call, and the transition into a nested-call target state before that nested call's start, with trace sequence numbers in that same order (verifying [[playbook-runtime-73](#playbook-runtime-73)] and [[playbook-runtime-37](#playbook-runtime-37)]).
...
The matrix shall further fail unless an artifact-schema-3 governed question outcome declared `unchanged` publishes only after a matching complete physical receipt, creates no logical operation or restoration state, and routes one valid answer through the ordinary authored continuation as a second separately governed boundary while a nonmatching initial receipt publishes no question and starts no continuation; nongoverned Boss-question fixtures shall retain their existing behavior (verifying [[playbook-runtime-73](#playbook-runtime-73)] and [[playbook-runtime-77](#playbook-runtime-77)]).
When one source state offers both an authored `unchanged` question and canonical `deferred` question, the real-host restore matrix shall fail unless both selected outcomes continue correctly with a saved player token and with fresh fallback, only the deferred outcome retains a cumulative operation, a newer owned boundary blocks use of older matching evidence, role or runtime-ownership mismatch and changed retained question text authorize no continuation, an existing open operation retains its authority despite a later matching standalone boundary, checkpoint mismatch starts no player, and a missing canonical operation remains rejected (verifying [[playbook-runtime-73](#playbook-runtime-73)]).
...
When the runtime integration suite drives an effect-authorized call over a baseline projection holding staged, modified, and untracked pre-existing entries, it shall fail unless the recorded call prompt ends with the pre-existing-changes block naming each path under its group and the rule, an empty baseline projection yields no block, a baseline of more than 40 paths names 40 and the remainder count, and an exclusively-`unchanged` call receives no block (verifying [[playbook-runtime-94](#playbook-runtime-94)]).
