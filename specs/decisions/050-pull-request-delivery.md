...
Amends [DR-016](016-script-actors-and-optimize-pass.md): a script blockquote may carry a `<placeholder>` relaying a runtime value the machine retains.
...
- A script's output never enters context, so nothing a script learns — the merge commit in particular — can become an output field.
- `gh` infers the pull request from the checked-out branch, but the run does not own that checkout: `PR-3` suspends across Boss turns while `code` works, so a step after it can resolve a different branch's pull request and deliver it while the result names the original.
...
- PR's remaining steps are mechanical and are authored as direct-Captain items whose prompt is exactly one static POSIX command: wait for checks (`PR-2`), publish the fix (`PR-4`), wait again (`PR-5`), merge (`PR-6`), and fast-forward the local default branch (`PR-7`).
  `PR-4` and `PR-6` are the steps that act on the pull request itself — a push to its branch and an irreversible merge — so each first requires the pull request `gh` infers to be exactly the one `PR-1` published, relaying that URL through the `<pull-request-url>` placeholder the source already established for `PR-3`.
  That amends DR-016's placeholder-free script text: a target the command cannot otherwise name is the one runtime value a script may carry, and an absent value leaves the quoted placeholder so the step refuses rather than acting unbound.
  The URL is Coder-reported text, so it binds as a single-quoted shell literal rather than being interpolated into the command: interpolated raw, a reported `$(…)` would both execute and expand to whatever makes the comparison succeed, turning the guard into the vulnerability it exists to prevent.
