...
- PR's remaining steps are mechanical and are authored as direct-Captain items whose prompt is exactly one static POSIX command: wait for checks (`PR-2`), publish the fix (`PR-4`), wait again (`PR-5`), merge (`PR-6`), and fast-forward the local default branch (`PR-7`).
  ...
  That amends DR-016's placeholder-free script text: a target the command cannot otherwise name is the one runtime value a script may carry, and an absent value leaves the placeholder literal so the step refuses rather than acting unbound.
