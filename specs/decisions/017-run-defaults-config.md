...
`playbook run` binds every required role and the captain to bare adapter `claude` unless each invocation repeats `--player`/`--captain` flags ([PBCLI-19](../user/playbook-cli.md#pbcli-19)).
The durable lineup a user already tuned lives in the top-level config at `${XDG_CONFIG_HOME:-$HOME/.config}/playbook/playbook.config.yaml` ([PBCLI-3](../user/playbook-cli.md#pbcli-3)), but the one-shot host never reads it: its `profiles`/`playbooks` model is keyed by enabled playbook, while `playbook run` deliberately runs modules that need no config block ([PBCLI-18](../user/playbook-cli.md#pbcli-18)).
...
Every value uses the exact `<adapter>[:<model>][@<effort>]` agent-string grammar of the run flags ([PBCLI-19](../user/playbook-cli.md#pbcli-19)) — one grammar whether an agent is bound by flag or by config.
...
Resume is untouched: config defaults bind only a first run, and `playbook run resume` rebuilds the parked lineup exactly from the session record ([DR-014 §3](014-durable-one-shot-run-sessions.md#3-one-shot-session-store)), even when the config changed after parking.
...
Effort support is validated through the same adapter-scoped path as flag-bound specs ([PBCLI-26](../dev/playbook-cli.md#pbcli-26)).
