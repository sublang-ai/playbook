...
2. **Retention captures the last unfinished pre-terminal generation, atomically with its nested stack.**
   ...
   For an outcome that would otherwise retain, when the root stack selected by that outcome contains a capability-less descendant, Captain retains the last complete turn-start generation or, when the turn began with such a descendant already live or the root began during the turn, emits no update, and Captain never retains the partial stack; clean completion still clears retention.
...
3. **A retained generation may cross sessions under an exact structural envelope and single-transfer ownership.**
   ...
   A fully validated nonresumable record preserves a trustworthy ordering boundary: only a settled same-working-directory boundary participates, a newest such boundary declines adoption without falling through, and an older or different-directory boundary does not prevent adoption from a newer resumable predecessor.
   A store-owned record that fresh discovery cannot validate leaves its working directory and predecessor order unproved, so the launch diagnoses and preserves it, publishes an empty target after every valid settled same-working-directory predecessor boundary it can observe, and does not fall through to older work.
...
- A capability-less descendant can prevent the current stack from becoming a retained generation but does not itself erase an earlier complete generation or block ordinary same-session snapshot continuation; clean completion still clears retention.
