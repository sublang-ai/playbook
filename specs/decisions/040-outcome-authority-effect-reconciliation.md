...
Runtime snapshots, trace events, and complete shell snapshots shall advance from schema `3` to schema `4`; the Captain session record shall advance to schema `6` because schema `4` already names the historical retained-generation shape and schema `5` was used by two incompatible pre-release shapes around the required `unresolvedEffects` addition.
...
- Playbook 9 record schema `3`, the historical record schema `4` retention shape, and both pre-release schema-`5` shapes predate the canonical schema-`6` record boundary; the earlier schema-`5` shape also lacks `unresolvedEffects`, so their files remain stored but cannot resume after the cutover, explicit selection rejects them, and implicit continuation and unrelated fresh discovery report and skip each recognized record with an archive-or-remove remedy rather than guessing whether governed work may replay.
