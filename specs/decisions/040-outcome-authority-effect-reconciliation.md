...
- Playbook 9 record schema `3`, the historical record schema `4` retention shape, and the transitional schema-`5` shape lacking `unresolvedEffects` predate complete authoritative effect settlement, so their files remain stored but cannot resume after the cutover; explicit selection rejects them, while implicit continuation and unrelated fresh discovery report and skip each recognized record with an archive-or-remove remedy rather than guessing whether governed work may replay.
