...
Accepted.
Pins the CODE-specific bindings that
[slc/link.md](../../slc/link.md) leaves open.

## Context

[slc/link.md](../../slc/link.md) is host- and FSM-agnostic.
The CODE playbook needs concrete choices for player binding,
...
role cligents, observer dispatch) with no awareness of playbooks,
XState, or `PlaybookRuntime`; this repo imports cligent as a dependency
and supplies the small adapter that satisfies cligent's `Captain`
interface.
A future presentation layer (web/Electron/CI) gets its own small
adapter file alongside the CODE artifacts — same pattern, different
host primitives.
...
| Input | Value |
| --- | --- |
| FSM artifact | `reference/sdlc/code.playbook/code.fsm.ts` (imports `codingMachine`, `CaptainInput`, `CaptainOutput`, `CodingInput`, `CodingEvent`) |
...
All four are recorded verbatim in the emitted file's top-of-file header
per [link.md §Output](../../slc/link.md#output).

`PlaybookRuntimeOptions` carries only per-run identity strings
(`coderPlayer`, `reviewerPlayer`) substituted into `<coder-llm>` /
`<reviewer-llm>` placeholders.
Player binding is *not* a runtime option; future per-run remapping
would be a separate DR/IR.

### 2. Player binding for CODE

CODE declares `Coder`, `Reviewer`, and the alias `Committer = Coder | Reviewer`
...
Composite states resolve `Committer` per source item via the populated
...
The runtime never speaks any host role id; `coder` / `reviewer` are
opaque strings the host adapter routes to its primitives.
...
The LLM-classifier prompt is fixed text the linker emits once: names the
four event types, the placeholder set for each payload field, demands
JSON, no FSM internals beyond the event union.

`BOSS_INTERRUPT` is reached only through explicit `/interrupt` (or the
LLM-classifier picking it).
...
CODE's `result` maps are self-describing
(gears2fsm round-2 review made them so), so each invocation's judge
prompt is the literal `result` map XState hands the call via
`CaptainInput.result`.
...
The runtime does not paraphrase or shorten result descriptions; every
CODE state uses LLM-judge.
...
source-item prompt verbatim (placeholders intact per gears2fsm).
...
Boss intent, review items, rebuttals, and task descriptions are prepended
as labelled blocks when set:
...
never used for player turns.
Hosts typically wire it to the host's Captain LLM (cligent:
`context.callCaptain`).
...
Player prompts and adjudicator JSON ride the host's record channels
(cligent's `captain_*` / `role_*`); the runtime shall not duplicate them
as telemetry.

### 10. Emitted module — `code.playbook.ts`

The link compiler emits exactly one file at
`reference/sdlc/code.playbook/code.playbook.ts` with a top-of-file
header recording the linker invocation:
...
Internal capabilities (names illustrative): player-prompt composer,
Boss-event classifier, LLM judge, player-id resolver, captain bridge.

### 11. Host adapter — tmux-play

File: `reference/sdlc/code.playbook/code.tmux-play.ts`.
It:
...
The adapter is ~40 lines once helpers factor out and is playbook-specific
only via the `./code.playbook.js` import.

**Role-id constraint.**
Because the player binding is baked at link time (§1), the user's
`tmux-play.config.yaml` `roles[]` shall declare role IDs that match the
baked `playerId` strings (`coder`, `reviewer`).
The adapter does not remap.

**Build / ESM constraint.**
cligent's session imports `captain.from` via native `import()`; native
ESM rejects `.ts` without a loader hook, so the adapter must exist as
compiled `.js` at the path `captain.from` references.
The package ships a TypeScript → ESM `.js` build emitting
`code.playbook.js` and `code.tmux-play.js` next to the `.ts` sources;
`package.json` declares `"type": "module"`; `.ts` sources use
...
After release as `@sublang/playbook`, `captain.from` swaps to the
package specifier (e.g. `'@sublang/playbook/code/tmux-play'`, final
form confirmed at publish time); roles, options, and the rest of the
