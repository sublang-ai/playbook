...
- [x] New [[playbook-runtime-47](../packages/playbook-runtime.md#playbook-runtime-47)] and matching `slc/link.md` contract: a host-reported direct-Captain result failure routes through the FSM error path to `failed` instead of rejecting the public boundary.
...
- [x] [[playbook-runtime-46](../packages/playbook-runtime.md#playbook-runtime-46)] re-scoped to what its own integration suite can drive.
- [x] `[Unreleased]` CHANGELOG entries for these changes, plus the missing `PlaybookRuntimeSnapshot.sequences.captainCall` note in the `1.3.0` section where that public member shipped, per [[release-4](../packages/release.md#release-4)].

## Tasks

1. **Spec surface.** _[done]_ Add playbook-runtime-47, align the direct-Captain result policy and runtime-owned `bossEvents` exclusion in `slc/link.md`, drop the unit-only clause from playbook-runtime-46, and add this record and its map row.
...
- No item changed for the `bossEvents` construction-time validation.
  [DR-019](../decisions/019-shared-linked-runtime-factory.md) §2 already requires conflicting duplicates to fail factory construction; a supplied classifier had short-circuited the only enforcement of it.
...
- playbook-runtime-47 deliberately changes the prior `slc/link.md` policy, which treated a structured non-abort direct-Captain result failure as control plane, to match the delegated-player boundary ([[playbook-runtime-9](../packages/playbook-runtime.md#playbook-runtime-9)]).
  ...
  Because this changes `handleBossInput` from rejection to a resolved outcome on the public semver-stable engine surface, the changelog marks it breaking and the next release shall use a new major version per [[release-1](../packages/release.md#release-1)] and [[release-15](../packages/release.md#release-15)].
- playbook-runtime-46's `captainCall` clause is removed rather than re-scoped.
  Its stated scope is the integration suite over real linked runtimes, and no shipped runtime both implements `exportSnapshot` and invokes a `captain` actor: CODE and DISCUSS have no direct-Captain invoke, and [[playbook-runtime-45](../packages/playbook-runtime.md#playbook-runtime-45)] forbids the compiled Captain the durable capability.
  Only the synthetic factory unit tests can drive it, and [[meta-21](../meta.md#meta-21)] forbids unit tests as spec items.
  The behavior stays specified by playbook-runtime-45, which already carries it, and stays covered by the factory unit tests.
- The factory's own tests stay unspecified for the same reason: `src/xstate-playbook-runtime.test.ts` is the DR-019 unit suite, so playbook-runtime-47 gains no test item.
