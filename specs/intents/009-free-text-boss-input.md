...
- [x] [DR-004](../decisions/004-link-code-fsm-to-playbook-runtime.md) — the CODE Boss-event mapping follows `slc/link.md`'s free-text classification; the `/start` … `/interrupt` slash mapping is dropped.
- [x] [DR-005](../decisions/005-boss-reply-suspension-path.md) — the Context paragraph and `awaitBossReply` entry-event references reworded for free-text classification, with no `/start` … `/interrupt`.
- [x] [`specs/packages/playbook-runtime.md`](../packages/playbook-runtime.md) — playbook-runtime-1/playbook-runtime-2 reworked: all turn input is free text, judge-classified, plus the `awaitBossReply` reply-vs-directive rule.
- [x] [`specs/packages/playbook-runtime.md`](../packages/playbook-runtime.md) — playbook-runtime-7 reworked: `callJudge` sole classifier, `BOSS_INTERRUPT` target selection, `awaitBossReply` state-aware classification.
- [x] [`specs/packages/playbook-runtime.md`](../packages/playbook-runtime.md) — drop the slash-form test items; add judge-classification and `awaitBossReply` items.
...
2. **Spec amendments.**
   `slc/link.md` (Boss-event mapping → free-text-only, `/command` reservation); `slc/gears2fsm.md` (the `awaitBossReply` prose drops the slash spellings); DR-004 (CODE Boss-event mapping follows `slc/link.md`); DR-005 (Context and `awaitBossReply` references); PBRT user playbook-runtime-1/playbook-runtime-2; PBRT dev playbook-runtime-7; PBRT test items.
