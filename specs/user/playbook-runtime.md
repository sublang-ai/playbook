...
The `captain.from` adapter-module path, the
`from: @sublang/playbook/code/registry` enablement entry, and the
`code.coder` / `code.reviewer` host player ids are generated by the
generic `playbook` launcher and shall not be required to appear in
the user-edited generic config; this supersedes the
user-maintained-invariant framing of [PBRT-4](#pbrt-4).
