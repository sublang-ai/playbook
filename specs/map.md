...
| dev | [playbook-code.md](dev/playbook-code.md) | `playbook-code` shim: cligent CLI resolution, overlay-template seeding with Claude Opus 4.8 / Claude Opus 4.8 1m / GPT-5.5 defaults, Codex `.git` writablePaths and notification defaults, append-only notification migration with non-mapping no-op, readiness heuristic, config composition (base discovery, exact shell `captain.from`, role→`players[]` mapping, Committer-alias resolution, `theme`/`layout`/`notifications` inheritance, owned YAML serialization), Node engine floor |
