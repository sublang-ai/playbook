...
| DR-030 | [030-turn-transaction-and-prompt-provenance.md](decisions/030-turn-transaction-and-prompt-provenance.md) | One authoritative turn transaction per Boss turn — input, decision outcome, ordered not-applied/applied/unknown operation receipts, a settlement derived from outcome and receipts together, and a shown/not-shown/unknown delivery receipt reattempted only after proven non-delivery — held in one session log that gives a healthy conversation each new record once and a replacement the whole log, with prompt content carrying independent exposure and trust so a control identifier stays forbidden for the life of every conversation that received it |
