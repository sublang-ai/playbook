...
| DR-030 | [030-turn-transaction-and-prompt-provenance.md](decisions/030-turn-transaction-and-prompt-provenance.md) | The Boss turn as recorded evidence — settlement derived from the ordered operations its action performed, presented once under a reported delivery truth, and recorded once for the durable conversation, withdrawing CAPTAIN-34's "exactly one visible settlement" — and prompt fragments carrying role and trust, so forbidden-set membership follows role while escaping and bounding follow trust |
