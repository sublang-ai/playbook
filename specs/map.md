...
| [DR-005](decisions/005-boss-reply-suspension-path.md) | 005-boss-reply-suspension-path.md | Quiescent scalar or branch-local Boss-question suspension, identified `BOSS_REPLY` reentry with explicit Q+A context, abandonment cleanup, optional backend continuation, and DR-040's checkpoint-bound cumulative effect operation |
...
| [DR-010](decisions/010-playbook-session-tracing-and-resume.md) | 010-playbook-session-tracing-and-resume.md | Immutable per-runtime playbook session UUIDs, boundary-complete ordered trace schema v3 with distinct role/player identity, advanced by DR-040 to v4 accepted-outcome evidence, and explicit fresh/resume selection from authoritative adapter tokens |
...
| [DR-015](decisions/015-per-run-agent-tuning.md) | 015-per-run-agent-tuning.md | Repeatable `--with <path>` top-level config overlays for new or ordinarily reopened Captain sessions, including model, effort, and fast-mode tuning; uncertain retry retains exact attempted settings |
...
| [DR-021](decisions/021-inline-agent-settings.md) | 021-inline-agent-settings.md | Remove profiles: Captain and top-level session players carry inline agent settings, while playbook role bindings may override model, effort, and fast mode |
| [DR-022](decisions/022-runtime-compatibility-contract.md) | 022-runtime-compatibility-contract.md | Versioned artifact/engine compatibility through `RUNTIME_ABI`, `SUPPORTED_ARTIFACT_SCHEMAS`, factory `spec.compat`, and registry `artifactSchema`; DR-032 introduces schema-2 local roles, while DR-040 stages authority schema `3` under ABI `1` before removing schema `2` at the next-major gate |
...
| [DR-027](decisions/027-runtime-compatibility-from-cligent.md) | 027-runtime-compatibility-from-cligent.md | Delegate agent-runtime version and adapter-scoped fast-mode capability knowledge to cligent: Playbook derives floors, repairs, verdicts, and validation without maintaining its own support lists |
| [DR-028](decisions/028-empty-ok-result-re-ask.md) | 028-empty-ok-result-re-ask.md | One corrective re-ask on an empty `ok` player or direct-Captain result before failure-sink routing, with DR-040 requiring a complete durable `unchanged` receipt at a governed player boundary |
...
| [DR-031](decisions/031-shared-captain-session-front-ends.md) | 031-shared-captain-session-front-ends.md | Interactive and headless front ends host the same compiled Captain session, config, nested stack, presentation contract, and durable continuation; DR-041 makes bare headless continuation prefer the invoking working directory |
| [DR-032](decisions/032-explicit-roles-session-players.md) | 032-explicit-roles-session-players.md | Playbook-local roles bind explicitly to Captain-session players; compatible current model, effort, and fast-mode settings may change across durable continuation |
...
| [DR-038](decisions/038-universal-run-resumption.md) | 038-universal-run-resumption.md | Resumption as a capability-gated machine property — retained pre-terminal generations with their nested stacks, cross-session adoption under an exact structural envelope that excludes current model, effort, and fast mode, ledger-authoritative players, Captain resume selection, and DR-040's clear on unresolved abandonment |
...
| [DR-042](decisions/042-shared-session-store-and-replay-stream.md) | 042-shared-session-store-and-replay-stream.md | Shared session-store facade, frozen replay envelope and configured location; DR-049 adds portable manifests, shared lifecycle, local hints and migration |
