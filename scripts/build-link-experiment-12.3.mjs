...
verify(producer, expected.producer, 'machine-root namespace, canonical question-storage and task-input corrections');
...
const taskInputRules = ["The corresponding `invoke.input` object shall include that field beside `prompt`; storing it only in machine context does not satisfy the actor-input contract.\n", "Boss text supplied by an entry event shall not become a required machine-construction input unless Source independently requires that value before the first Boss turn; an optional source-appropriate seed may remain.\n"];
const producerBeforeTaskRules = taskInputRules.reduce((text, rule) => text.replace(rule, ''), producer);
assert.equal(producerBeforeTaskRules.replace(rootNamespaceRule, '').replace(questionStorageRule, ''), baseline['gears2fsm.md'], 'Only the reviewed root namespace, question-storage and task-input clarifications may change the FSM producer');
const rejectedProducer = producerBeforeTaskRules.replace(questionStorageRule, '');
